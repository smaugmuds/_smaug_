...
affect_modify (CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd)
{
  ...
  switch (location)
    {
    default:
      bug ("Affect_modify: unknown location %d. (%s)", paf->location,
	   ch->name);
      return;
...
/* spell cast upon wear/removal of an object	-Thoric */
	...
	return;
      ...
      mod = abs (mod);
      if (IS_VALID_SN (mod)
	  && (skill = skill_table[mod]) != NULL && skill->type == SKILL_SPELL)
	{
  ...
  if (!IS_NPC (victim)
      ...
      && get_trust (ch) < victim->pcdata->wizinvis)
    {
	  ...
	  (is_name (ch->name, victim->pcdata->see_me)
	   || (is_name ("room", victim->pcdata->see_me)
	       && (ch->in_room->vnum == victim->in_room->vnum))
	   || (ch->pcdata->council
	       && is_name (ch->pcdata->council->abbrev,
			   victim->pcdata->see_me)) || (ch->pcdata->clan
							&&
							is_name
							(ch->pcdata->clan->abbrev,
							 victim->pcdata->see_me))))
	return TRUE;
      else
	return FALSE;
    }
...
name_stamp_stats (CHAR_DATA * ch)
{
  ...
  for (x = 0; x < strlen (ch->name); x++)
    {
      c =
	(number_range (65, 122) + x + number_range (65, 122) -
	 number_range (65, 122) + number_range (65, 122));
      if (c < 0)
	c = 14;
      b = c % 15;
      if (x == 0)
  ...
  x =
    ch->perm_str + ch->perm_dex + ch->perm_wis + ch->perm_int + ch->perm_con +
...
name_stamp_stats (CHAR_DATA * ch)
{
  ...
  for (x = 0; x < strlen (ch->name); x++)
	...
	c =
	  (ch->name[0] + ch->name[strlen (ch->name) - 1] -
	   ch->name[strlen (ch->name) / 2]);
      else
	c =
	  (ch->name[x] + x + ch->name[x - 1] - ch->name[x / 2] +
	   ch->name[x + 1]);
      if (c < 0)
	c = 14;
      b = c % 15;
...
in_soft_range (CHAR_DATA * ch, AREA_DATA * tarea)
{
  if (IS_IMMORTAL (ch))
    ...
    if (ch->level >= tarea->low_soft_range
...
empty_obj (OBJ_DATA * obj, OBJ_DATA * destobj, ROOM_INDEX_DATA * destroom)
{
  ...
  if (destobj || (!destroom && !ch && (destobj = obj->in_obj) != NULL))
      ...
      for (otmp = obj->first_content; otmp; otmp = otmp_next)
	{
	  ...
	  if ((destobj->item_type == ITEM_CONTAINER
		  ...
		  destobj->value[0] || (destobj->in_room
					&& destobj->in_room->max_weight
					&& destobj->in_room->max_weight <
					get_real_obj_weight (otmp) +
					destobj->in_room->weight)))
	    continue;
	  obj_from_obj (otmp);
  ...
  if (destroom || (!ch && (destroom = obj->in_room) != NULL))
	...
	{
	  otmp_next = otmp->next_content;
	  if (destroom->max_weight
	      && destroom->max_weight <
	      get_real_obj_weight (otmp) + destroom->weight)
	    continue;
