 ...
 */
void
do_mptag (CHAR_DATA * ch, char *argument)
{
  CHAR_DATA *victim;
  VARIABLE_DATA *vd;
  char *p;
 ...
 */
void
do_mprmtag (CHAR_DATA * ch, char *argument)
{
  CHAR_DATA *victim;
  char *p;
  char arg1[MAX_INPUT_LENGTH];
 ...
 */
void
do_mpflag (CHAR_DATA * ch, char *argument)
{
  CHAR_DATA *victim;
  VARIABLE_DATA *vd;
  char *p;
 ...
 */
void
do_mprmflag (CHAR_DATA * ch, char *argument)
{
  CHAR_DATA *victim;
  VARIABLE_DATA *vd;
  char *p;
  ...
  for (;;)
	...
	case 'E':
	  if (!str_cmp (word, "End"))
		...
		default:
		  {
		    bug ("Fread_variable: invalid/incomplete variable %s",
			 pvd->tag);
		    if (pvd->tag)
		      DISPOSE (pvd->tag);
		    DISPOSE (pvd);
	...
	case 'I':
	  if (!str_cmp (word, "Int"))
	    {
	      if (pvd->type != vtINT)
		bug ("Fread_variable: Type mismatch -- type(%d) != vtInt",
		     pvd->type);
	      else
		{
	...
	case 'S':
	  if (!str_cmp (word, "Str"))
	    {
	      if (pvd->type != vtSTR)
		bug ("Fread_variable: Type mismatch -- type(%d) != vtSTR",
		     pvd->type);
	      else
		{
	...
	case 'X':
	  if (!str_cmp (word, "Xbit"))
	    {
	      if (pvd->type != vtXBIT)
		bug ("Fread_variable: Type mismatch -- type(%d) != vtXBIT",
		     pvd->type);
	      else
		{
