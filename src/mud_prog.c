/*
 ...
 [|             /  *********   *        *  *        *   ********    *******   |]
  \\____________\____________________________________________________________//
     ...
     |_____________________________________________________________________|
     |                                                                     |
     | The MUDprograms are heavily based on the original MOBprogram code   |
     | that was written by N'Atas-ha.	                                     |
     |                                                                     |
     | Much has been added, including the capability to put a "program" on |
     | rooms and objects, not to mention many more triggers and ifchecks.  |
     |                                                                     |
     ...
     |                                                                     |
     | Mudprog parsing has been rewritten (in mprog_driver).               |
     | Mprog_process_if and mprog_process_cmnd have been removed,          |
     | mprog_do_command is new. Full support for nested ifs is in.         |
     |_____________________________________________________________________|
    //                                                                     \\
...
args ((char ch, char *t, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj,
       CHAR_DATA * vict, OBJ_DATA * v_obj, CHAR_DATA * rndm));
void mprog_driver
args ((char *com_list, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj,
 ...
 */
bool
mprog_seval (char *lhs, char *opr, char *rhs, CHAR_DATA * mob)
{
  ...
  if (chkchar)
      ...
      if (!str_cmp (chck, "stopscript"))
	{
	  return (IS_NPC (chkchar)
		  && xIS_SET (chkchar->act, ACT_STOP_SCRIPT));
	}
	...
	{
	  return (!IS_NPC (chkchar) ?
		  mprog_veval (chkchar->pcdata->condition[COND_DRUNK], opr,
			       atoi (rval), mob) : FALSE);
	}
	...
	{
	  return ((IS_OUTSIDE (chkchar)
		   && chkchar->in_room->sector_type !=
		   SECT_INSIDE) ? FALSE : TRUE);
	}
      if (!str_cmp (chck, "nomagic"))
	{
	  return xIS_SET (chkchar->in_room->room_flags,
			  ROOM_NO_MAGIC) ? TRUE : FALSE;
	}
      if (!str_cmp (chck, "safe"))
	{
	  return xIS_SET (chkchar->in_room->room_flags,
			  ROOM_SAFE) ? TRUE : FALSE;
	}

      if (!str_cmp (chck, "nosummon"))
	{
	  return xIS_SET (chkchar->in_room->room_flags,
			  ROOM_NO_SUMMON) ? TRUE : FALSE;
	}
      if (!str_cmp (chck, "noastral"))
	{
	  return xIS_SET (chkchar->in_room->room_flags,
			  ROOM_NO_ASTRAL) ? TRUE : FALSE;
	}
      if (!str_cmp (chck, "nosupplicate"))
	{
	  return xIS_SET (chkchar->in_room->room_flags,
			  ROOM_NOSUPPLICATE) ? TRUE : FALSE;
	}
...
*/
	...
	{
	  return mprog_veval (get_timer (chkchar, TIMER_RECENTFIGHT), opr,
			      atoi (rval), mob);
	}
		  ...
		  CHAR_DATA * rndm, bool ignore, bool ignore_ors)
{
  ...
  if (!str_cmp (firstword, "if"))
	...
	validif =
	  mprog_do_ifcheck (ifcheck, mob, actor, obj, victim, target, rndm);
  ...
  while (mpsleep)		/* Find progs to continue */
      ...
      if (--mpsleep->timer <= 0)
	{
	  ...
	  mprog_driver (mpsleep->com_list, mpsleep->mob, mpsleep->actor,
			mpsleep->obj, mpsleep->victim, mpsleep->target,
			mpsleep->single_step);
	...
	if ((list[0] == 'p') && (list[1] == ' '))
	  {
	    ...
	    while ((start = strstr (dupl, list)))
		  ...
		  && (*(end = start + strlen (list)) == ' '
		      || *end == '\n' || *end == '\r' || *end == '\0'))
		{
		  mprog_driver (mprg->comlist, mob, actor, obj, victim,
				target, FALSE);
		  executed = TRUE;
		  break;
		}
		  ...
		  CHAR_DATA * victim, OBJ_DATA * target, int type)
{
  ...
  for (mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next)
	...
	{
	  mprg->triggered = TRUE;
	  mprog_driver (mprg->comlist, mob, actor, obj, victim, target,
			FALSE);
	}
    }
  return;
...
void
mprog_login_trigger (CHAR_DATA * ch)
{
...
void
mprog_void_trigger (CHAR_DATA * ch)
{
  CHAR_DATA *vmob, *vmob_next;
...
mprog_greet_trigger (CHAR_DATA * ch)
{
  ...
  if (vmob && mnum == MAX_MOB_ROOM)
    {
      char msg[100];
      sprintf (msg, "Greet_prog: too many mobs in room %d.",
	       ch->in_room->vnum);
      log_string (msg);
    }
  ...
  for (vmob = actor->in_room->first_person; vmob; vmob = vmob->next_in_room)
      ...
      if (IS_NPC (vmob) && HAS_PROG (vmob->pIndexData, SPEECH_PROG))
	{
	  if (IS_NPC (actor) && actor->pIndexData == vmob->pIndexData)
	    continue;
	  mprog_wordlist_check (txt, vmob, actor, NULL, NULL, NULL,
				SPEECH_PROG);
	}
    }
}
...
void
mprog_tell_trigger (char *txt, CHAR_DATA * actor)
{
  CHAR_DATA *vmob;
...
mprog_command_trigger (CHAR_DATA * actor, char *txt)
{
  ...
  for (vmob = actor->in_room->first_person; vmob; vmob = vmob->next_in_room)
      ...
      if (IS_NPC (vmob) && HAS_PROG (vmob->pIndexData, CMD_PROG))
	{
	  if (IS_NPC (actor) && actor->pIndexData == vmob->pIndexData)
	    continue;
	  if (mprog_wordlist_check
	      (txt, vmob, actor, NULL, NULL, NULL, CMD_PROG))
	    return TRUE;
	}
    }
  return FALSE;
  ...
  if (HAS_PROG (obj->pIndexData, SCRIPT_PROG))
      ...
      if (mprg->type == SCRIPT_PROG)
	{
	  if (mprg->arglist[0] == '\0'
	    ...
	    {
	      set_supermob (obj);
	      mprog_driver (mprg->comlist, supermob, NULL, NULL, NULL, NULL,
			    TRUE);
...
void
oprog_speech_trigger (char *txt, CHAR_DATA * ch)
{
  OBJ_DATA *vobj;
  ...
  for (vobj = ch->in_room->first_content; vobj; vobj = vobj->next_content)
    if (HAS_PROG (vobj->pIndexData, SPEECH_PROG))
      oprog_wordlist_check (txt, supermob, ch, vobj, NULL, NULL, SPEECH_PROG,
			    vobj);
...
oprog_command_trigger (CHAR_DATA * ch, char *txt)
{
  ...
  for (vobj = ch->in_room->first_content; vobj; vobj = vobj->next_content)
    if (HAS_PROG (vobj->pIndexData, CMD_PROG))
	  ...
	  (txt, supermob, ch, vobj, NULL, NULL, CMD_PROG, vobj))
	return TRUE;
		   ...
		   OBJ_DATA * targ, void *vo)
{
  ...
  if (HAS_PROG (obj->pIndexData, USE_PROG))
	...
	{
	  executed =
	    oprog_percent_check (supermob, ch, obj, vict, targ, USE_PROG);
	}
      else
	executed =
	  oprog_percent_check (supermob, ch, obj, NULL, NULL, USE_PROG);
      release_supermob ();
    }
  return executed;
}
...
void
rprog_login_trigger (CHAR_DATA * ch)
{
...
void
rprog_void_trigger (CHAR_DATA * ch)
{
...
void
rprog_imminfo_trigger (CHAR_DATA * ch)
{
...
void
rprog_death_trigger (CHAR_DATA * killer, CHAR_DATA * ch)
{
...
void
rprog_speech_trigger (char *txt, CHAR_DATA * ch)
{
  if (HAS_PROG (ch->in_room, SPEECH_PROG))
    {
      /* supermob is set and released in rprog_wordlist_check */
      rprog_wordlist_check (txt, supermob, ch, NULL, NULL, NULL, SPEECH_PROG,
			    ch->in_room);
    }
}
  ...
  if (HAS_PROG (ch->in_room, CMD_PROG))
    {
	  ...
	  (txt, supermob, ch, NULL, NULL, NULL, CMD_PROG, ch->in_room))
	return TRUE;
    }
  return FALSE;
}
...
void
progbug (char *str, CHAR_DATA * mob)
{
  char buf[MAX_STRING_LENGTH];
      ...
      while ((mpact = room->mpact) != NULL)
	{
	  if (mpact->ch->in_room == room)
	    rprog_wordlist_check (mpact->buf, supermob, mpact->ch, mpact->obj,
				  mpact->victim, mpact->target, ACT_PROG,
				  room);
