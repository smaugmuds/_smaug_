	...
	if (get_trust (ch) >= obj->level)
	...
	else
	  {
	    sprintf (buf, "%s had on %s:  ch->level = %d  obj->level = %d",
		     ch->name, obj->name, ch->level, obj->level);
	    bug (buf, 0);
	  }
	unequip_char (ch, obj);
      }
...
save_char_obj (CHAR_DATA * ch)
{
  ...
  if (IS_SET (sysdata.save_flags, SV_BACKUP) ||
    ...
    {
      sprintf (strback, "%s%c/%s", BACKUP_DIR,
  ...
  if (ch->level >= LEVEL_IMMORTAL
      || IS_SET (ch->pcdata->flags, PCFLAG_RETIRED))
    {
...
fwrite_char (CHAR_DATA * ch, FILE * fp)
{
  ...
  if (pos == POS_BERSERK
      || pos == POS_AGGRESSIVE
      || pos == POS_FIGHTING || pos == POS_DEFENSIVE || pos == POS_EVASIVE)
    pos = POS_STANDING;
  pos += 100;
 ...
 */
bool
load_char_obj (DESCRIPTOR_DATA * d, char *name, bool preload)
{
  char strsave[MAX_INPUT_LENGTH];
  CHAR_DATA *ch;
  FILE *fp;
	  ...
	  if (!strcmp (word, "Skill"))
	      ...
	      else
		{
		  ...
		  if (file_ver < 3)
		    ...
		    sn =
		      bsearch_skill_exact (sword, gsn_first_skill,
					   gsn_first_weapon - 1);
		  if (sn < 0)
		    bug ("Fread_char: unknown skill.", 0);
		  else
	  ...
	  if (!strcmp (word, "Spell"))
	      ...
	      else
		{
		  ...
		  if ((sword = fread_word (fp)) == NULL)
		    return -1;
		  sn =
		    bsearch_skill_exact (sword, gsn_first_spell,
					 gsn_first_skill - 1);
		  if (sn < 0)
	  ...
	  if (!strcmp (word, "Tongue"))
	      ...
	      else
		{
		  ...
		  if ((sword = fread_word (fp)) == NULL)
		    return -1;
		  sn =
		    bsearch_skill_exact (sword, gsn_first_tongue,
					 gsn_top_sn - 1);
		  if (sn < 0)
	  ...
	  if (!strcmp (word, "Weapon"))
	      ...
	      else
		{
		  ...
		  if ((sword = fread_word (fp)) == NULL)
		    return -1;
		  sn =
		    bsearch_skill_exact (sword, gsn_first_weapon,
					 gsn_first_tongue - 1);
		  if (sn < 0)
...
void
fread_obj (CHAR_DATA * ch, FILE * fp, sh_int os_type)
{
  OBJ_DATA *obj;
  char *word;
  char buf[MAX_STRING_LENGTH];
	  ...
	  if (!strcmp (word, "End"))
	      ...
	      if (obj->item_type == ITEM_HOUSEKEY)
		{
		  if (!check_owner (obj))
		    {
		      bug
			("Fread_obj: house key found not belonging to correct house",
			 0);
		      if (obj->name)
			STRFREE (obj->name);
		      if (obj->description)
...
do_last (CHAR_DATA * ch, char *argument)
{
  ...
  if (get_trust (ch) < LEVEL_ASCENDANT
	  ...
	  || !str_cmp (arg, "Nivek")
	  || !str_cmp (arg, "Kali")
	  || !str_cmp (arg, "Blodkai") || !str_cmp (arg, "Edmond")))
    {
...
void
write_corpses (CHAR_DATA * ch, char *name, OBJ_DATA * objrem)
{
  OBJ_DATA *corpse;
  FILE *fp = NULL;
