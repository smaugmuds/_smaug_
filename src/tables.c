...
spell_function (char *name)
{
  ...
  if (!str_cmp (name, "spell_close_portal"))
    return spell_close_portal;
...
#ifdef MYSTARIC
  if (!str_cmp (name, "spell_group_teleport"))
    return spell_group_teleport;
#endif
...
skill_function (char *name)
{
  switch (LOWER (name[3]))
	...
	return do_add_change;
      if (!str_cmp (name, "do_add_imm_news"))
	return do_add_imm_news;
      if (!str_cmp (name, "do_add_news"))
	return do_add_news;
      if (!str_cmp (name, "do_adminlist"))
	return do_adminlist;
      if (!str_cmp (name, "do_advance"))
	return do_advance;
      if (!str_cmp (name, "do_add_imm_host"))
	return do_add_imm_host;
      if (!str_cmp (name, "do_aecho"))
	return do_aecho;
	...
	return do_casinoset;
      if (!str_cmp (name, "do_casinostat"))
	return do_casinostat;
      if (!str_cmp (name, "do_casinos"))
	return do_casinos;
#endif
	...
	return do_compare;
      if (!str_cmp (name, "do_compass"))
	return do_compass;
      if (!str_cmp (name, "do_condition"))
	return do_condition;
	...
	return do_cwho;
      break;
    case 'd':
      if (!str_cmp (name, "do_defeats"))
	return do_defeats;
      if (!str_cmp (name, "do_deities"))
	return do_deities;
      if (!str_cmp (name, "do_delay"))
	return do_delay;
      if (!str_cmp (name, "do_delete_char"))
	return do_delete_char;
	...
	return do_equipment;
      if (!str_cmp (name, "do_equipment_full"))
	return do_equipment_full;
      if (!str_cmp (name, "do_examine"))
	return do_examine;
      if (!str_cmp (name, "do_exits"))
	return do_exits;
      if (!str_cmp (name, "do_extinguish"))
	return do_extinguish;
      break;
    case 'f':
      if (!str_cmp (name, "do_favor"))
	return do_favor;
      if (!str_cmp (name, "do_feed"))
	return do_feed;
      if (!str_cmp (name, "do_fill"))
	return do_fill;
      if (!str_cmp (name, "do_findexit"))
	return do_findexit;
	...
	return do_hide;
      if (!str_cmp (name, "do_hintedit"))
	return do_hintedit;
      if (!str_cmp (name, "do_hints"))
	return do_hints;
      if (!str_cmp (name, "do_hintsubmit"))
	return do_hintsubmit;
	...
	return do_make;
      if (!str_cmp (name, "do_makeadminlist"))
	return do_makeadminlist;
      if (!str_cmp (name, "do_makeboard"))
	return do_makeboard;
#ifdef MYSTARIC
      if (!str_cmp (name, "do_makecasino"))
	return do_makecasino;
#endif
	...
	return do_makerepair;
      if (!str_cmp (name, "do_makeretiredlist"))
	return do_makeretiredlist;
	...
	return do_meditate;
      if (!str_cmp (name, "do_memberlist"))
	return do_memberlist;
      if (!str_cmp (name, "do_memory"))
	return do_memory;
      if (!str_cmp (name, "do_message"))
	return do_message;
	...
	return do_mp_fill_in;
      if (!str_cmp (name, "do_mphunt"))
	return do_mphunt;
      if (!str_cmp (name, "do_mphate"))
	return do_mphate;
	...
	return do_mpfavor;
      if (!str_cmp (name, "do_mpflag"))
	return do_mpflag;
      if (!str_cmp (name, "do_mptag"))
	return do_mptag;
      if (!str_cmp (name, "do_mprmflag"))
	return do_mprmflag;
      if (!str_cmp (name, "do_mprmtag"))
	return do_mprmtag;
...
*/
	...
	return do_name;
      if (!str_cmp (name, "do_nanny_help"))
	return do_nanny_help;
	...
	return do_noauction;
      if (!str_cmp (name, "do_nobeckon"))
	return do_nobeckon;
      if (!str_cmp (name, "do_nobio"))
	return do_nobio;
      if (!str_cmp (name, "do_nodesc"))
	return do_nodesc;
      if (!str_cmp (name, "do_noemote"))
	return do_noemote;
      if (!str_cmp (name, "do_nohomepage"))
	return do_nohomepage;
      if (!str_cmp (name, "do_nohttp"))
	return do_nohttp;
...
/*	if ( !str_cmp( name, "do_opentourney" ))	return do_opentourney; */
	...
	return do_retire;
      if (!str_cmp (name, "do_retiredlist"))
	return do_retiredlist;
      if (!str_cmp (name, "do_retiredtalk"))
	return do_retiredtalk;
      if (!str_cmp (name, "do_retran"))
	return do_retran;
      if (!str_cmp (name, "do_return"))
	return do_return;
      if (!str_cmp (name, "do_rgrid"))
	return do_rgrid;
	...
	return do_strip;
      if (!str_cmp (name, "do_stset"))
	return do_stset;
      if (!str_cmp (name, "do_ststat"))
	return do_ststat;
...
skill_name (DO_FUN * skill)
{
  ...
  if (skill == do_add_imm_news)
    return "do_add_imm_news";
  if (skill == do_add_news)
    return "do_add_news";
  if (skill == do_adminlist)
    return "do_adminlist";
  if (skill == do_advance)
    return "do_advance";
  if (skill == do_aecho)
  ...
  if (skill == do_equipment)
    return "do_equipment";
  if (skill == do_equipment_full)
    return "do_equipment_full";
  if (skill == do_examine)
    return "do_examine";
  if (skill == do_exits)
    return "do_exits";
  if (skill == do_extinguish)
    return "do_extinguish";
  if (skill == do_favor)
    return "do_favor";
  if (skill == do_feed)
  ...
  if (skill == do_hide)
    return "do_hide";
  if (skill == do_hintedit)
    return "do_hintedit";
  if (skill == do_hints)
    return "do_hints";
  if (skill == do_hintsubmit)
    return "do_hintsubmit";
  if (skill == do_hitall)
    return "do_hitall";
  if (skill == do_hl)
  ...
  if (skill == do_make)
    return "do_make";
  if (skill == do_makeadminlist)
    return "do_makeadminlist";
  if (skill == do_makeboard)
    return "do_makeboard";
#ifdef MYSTARIC
  if (skill == do_makecasino)
    return "do_makecasino";
#endif
...
/*  if ( skill == do_opentourney )	return "do_opentourney"; */
  ...
  if (skill == do_retire)
    return "do_retire";
  if (skill == do_retiredlist)
    return "do_retiredlist";
  if (skill == do_retiredtalk)
    return "do_retiredtalk";
  if (skill == do_retran)
    return "do_retran";
  if (skill == do_return)
  ...
  for (;;)
	...
	case 'E':
	  if (!str_cmp (word, "End"))
		...
		{
		  sprintf (buf,
			   "Load_class_file: Class (%s) bad/not found (%d)",
			   class->
			   who_name ? class->who_name : "name not found", cl);
		  bug (buf, 0);
		  if (class->who_name)
	...
	case 'S':
	  if (!str_cmp (word, "Skill"))
		...
		{
		  sprintf (buf,
			   "load_class_file: Skill %s -- class bad/not found (%d)",
			   word, cl);
		  bug (buf, 0);
		}
	...
	case 'T':
	  if (!str_cmp (word, "Title"))
	      ...
	      if (cl < 0 || cl >= MAX_CLASS)
		{
		  ...
		  sprintf (buf,
			   "load_class_file: Title -- class bad/not found (%d)",
			   cl);
		  bug (buf, 0);
  ...
  for (;;)
	...
	case 'E':
	  if (!str_cmp (word, "End"))
		...
		{
		  sprintf (buf,
			   "Load_race_file: Race (%s) bad/not found (%d)",
			   race->
			   race_name ? race->race_name : "name not found",
			   ra);
		  bug (buf, 0);
		  DISPOSE (race);
		  return FALSE;
	...
	case 'S':
	  ...
	  if (!str_cmp (word, "Skill"))
		...
		{
		  sprintf (buf,
			   "load_race_file: Skill %s -- race bad/not found (%d)",
			   word, ra);
		  bug (buf, 0);
		}
	      else if (!IS_VALID_SN (sn))
	...
	case 'W':
	  ...
	  if (!str_cmp (word, "WhereName"))
	      ...
	      if (ra < 0 || ra >= MAX_RACE)
		{
		  ...
		  sprintf (buf,
			   "load_race_file: Title -- race bad/not found (%d)",
			   ra);
		  bug (buf, 0);
		  tmp = fread_string_nohash (fp);
		  DISPOSE (tmp);
...
/* just added */
	...
	case 'E':
	  if (!str_cmp (word, "End"))
		...
		{
		  bug
		    ("fread_skill(%s):  Has saving throw (%d) with no saving effect.",
  ...
  if ((fp = fopen (HERB_FILE, "r")) != NULL)
      ...
      for (;;)
	{
	  ...
	  if (!str_cmp (word, "HERB"))
		...
		{
		  bug ("load_herb_table: more herbs than MAX_HERB %d",
		       MAX_HERB);
		  fclose (fp);
		  return;
		}
...
bool
fread_storage (int rnum, char *filename)
{
  FILE *fp;
  bool found;
