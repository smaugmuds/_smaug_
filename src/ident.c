...
void
auth_maxdesc (int *md, fd_set * ins, fd_set * outs, fd_set * excs)
{
  AUTH_DATA *a;
...
void
auth_check (fd_set * ins, fd_set * outs, fd_set * excs)
{
  AUTH_DATA *a, *a_next;
  CHAR_DATA *ch;
  bool ferr;
...
bool
auth_read (AUTH_DATA * a, CHAR_DATA * ch)
{
...
bool
auth_write (AUTH_DATA * a, CHAR_DATA * ch)
{
  char authbuf[32];
  int n;
...
bool
auth_open (AUTH_DATA * a, CHAR_DATA * ch)
{
  ...
  if (getsockname (d->descriptor, (struct sockaddr *) &us, &ulen) < 0)
    {
      ...
      ENDRET ("Set_auth: getsockname error for %s@%s.",
	      "(getsockname error)");
    }
  if (getpeername (d->descriptor, (struct sockaddr *) &them, &tlen) < 0)
    {
      perror ("set_auth: getpeername");
      ENDRET ("Set_auth: getpeername error for %s@%s.",
	      "(getpeername error)");
    }
