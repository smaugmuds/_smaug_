...
void
do_renumber (CHAR_DATA * ch, char *argument)
{
  RENUMBER_AREA *r_area;
  AREA_DATA *area;
  bool is_proto;
...
RENUMBER_DATA *
gather_one_list (sh_int type, int low, int high, int new_base, bool fill_gaps,
		 int *max_vnum)
	  ...
	  if (exit->rvnum != i)
	    {
	      ...
	      rev_exit =
		get_exit_to (exit->to_room, rev_dir[exit->vdir], old_vnum);
...
void
warn_in_prog (CHAR_DATA * ch, int low, int high, char *where, int vnum,
	      MPROG_DATA * mprog, RENUMBER_AREA * r_area)
{
