...
bool
can_astral_room (ROOM_INDEX_DATA * room)
{
...
skill_lookup (const char *name)
{
  ...
  if ((sn =
       bsearch_skill_exact (name, gsn_first_spell,
			    gsn_first_skill - 1)) == -1)
    if ((sn =
	 bsearch_skill_exact (name, gsn_first_skill,
			      gsn_first_weapon - 1)) == -1)
      if ((sn =
	   bsearch_skill_exact (name, gsn_first_weapon,
				gsn_first_tongue - 1)) == -1)
	if ((sn =
	     bsearch_skill_exact (name, gsn_first_tongue,
				  gsn_top_sn - 1)) == -1)
	  if ((sn =
  ...
  for (;;)
    {
      ...
      if (LOWER (name[0]) == LOWER (skill_table[sn]->name[0])
	  && (!str_prefix (name, skill_table[sn]->name)
	      || (sysdata.magichell && ch->level > 35
		  && number_percent () < 30)) && ch->pcdata->learned[sn] > 0
	  && ch->level >= skill_table[sn]->skill_level[ch->class])
	return sn;
      if (first >= top)
	return -1;
...
do_cast (CHAR_DATA * ch, char *argument)
{
  ...
  switch (ch->substate)
	...
	mana =
	  IS_NPC (ch) ? 0 : UMAX (skill->min_mana * number_range (5, 10) / 5,
				  (100 /
				   (2 + ch->level -
				    skill->skill_level[ch->class])) *
				  number_range (5, 10) / 5);
...
spell_charm_person (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  if (IS_AFFECTED (victim, AFF_CHARM)
      ...
      || !can_charm (ch) || saves_spell_staff (chance, victim))
    {
      failed_casting (skill, ch, victim, NULL);
      return rSPELL_FAILED;
    }
...
spell_disenchant_weapon (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  if (xIS_SET (obj->pIndexData->extra_flags, ITEM_MAGIC))
    {
      send_to_char
	("You can't disenchant a weapon that's inherently magical.", ch);
      return rSPELL_FAILED;
    }
  ...
  if (xIS_SET (obj->pIndexData->extra_flags, ITEM_ANTI_GOOD)
    ...
    {
      send_to_char
	("You can't disenchant a weapon that's inherently good or evil.", ch);
      return rSPELL_FAILED;
    }
...
bool
can_locate_obj (CHAR_DATA * ch, OBJ_DATA * obj)
{
...
bool
can_locate_mob (CHAR_DATA * ch, CHAR_DATA * victim)
{
  bool ch_immort;
  ...
  if (IS_NPC (victim))
	...
	return FALSE;
      if (victim->timer > 1 && !IS_NPC (ch) && IS_PKILL (victim) && !ch_immort
	  && !victim->desc)
	return FALSE;
...
char *
cont_descr (void *cont, ct_type t)
{
  CHAR_DATA *mob;
...
void
build_pairs (CHAR_DATA * ch, OBJ_DATA * obj, void *ctn, ct_type ctp,
	     char *ct_name, int nest)
{
  KEYW_NODE *keyw;
  PAIRS_NODE *pair;
  int idx, pkey;
  ...
  for (; obj; obj = obj->next_content)
      ...
      if (can_locate_obj (ch, obj))
	{
	  ...
	  if (keyw ? keyw->ok : nifty_is_name (target_name, obj->name))
		...
		if (pair->desc == obj->short_descr && pair->cont == ct_name
		    && pair->ct == ctp)
		  break;
  ...
  for (x = 0; x < MAX_KEY_HASH; x++)
      ...
      for (room = room_index_hash[x]; room; room = room->next)
	{
	  if (xIS_SET (room->room_flags, ROOM_NOLOCATE)
	    ...
	    continue;
	  build_pairs (ch, room->first_content, room, CTN_ROOM, room->name,
		       1);
	  for (mob = room->first_person; mob; mob = mob->next_in_room)
	    if (can_locate_mob (ch, mob))
	      build_pairs (ch, mob->first_carrying, mob, CTN_MOB,
			   cont_descr (mob, CTN_MOB), 1);
	}
    }
  if (pair_overflow)
    {
      sprintf (msg, "Pair overflowed by %d locating %s.", pair_overflow,
	       target_name);
      log_string (msg);
    }
  free (keyw_vec);
      ...
      if (x)
	{
	  if (x < 3)
		...
		("\n\r...ENOUGH!  Your mortal mind was not meant for omniscience.\n\r",
		 ch);
	      send_to_char
		("What is left of it may need time to recover from this ordeal...\n\r",
		 ch);
	    }
	  x = ch->mental_state < 0 ? -x : x;
	  ch->mental_state =
	    URANGE (-ch->level, ch->mental_state + x / 5, ch->level);
	}
    }
  return rNONE;
}
...
ch_ret
spell_group_teleport (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  CHAR_DATA *nch, *nch_next;
  ...
  act (AT_MAGIC,
       "$n's arms rise towards the sky, a tornado engulfs the area.", victim,
       NULL, NULL, TO_ROOM);
  act (AT_MAGIC, "$n's group is sucked into the raging tornadoes.", victim,
       NULL, NULL, TO_ROOM);
  ...
  for (nch = start->first_person; nch; nch = nch_next)
    {
      nch_next = nch->next_in_room;
      if (is_same_group (nch, victim)
	  && (in_hard_range (nch, pRoomIndex->area)))
	{
	  char_from_room (nch);
...
spell_ventriloquate (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  if (speaker[0] == '\0')
    {
      send_to_char
	("Your voice sputters as you try to cast it in random directions.\n\r",
	 ch);
      return rSPELL_FAILED;
    }
...
ch_ret
spell_close_portal (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  for (obj = ch->in_room->first_content; obj; obj = obj_next)
      ...
      if (obj->item_type == ITEM_PORTAL)
	{
	  ...
	  extract_obj (obj);
	  act (AT_MAGIC,
	       "As $n utters a lengthy incantation, the portal spins into nothingness.",
	       ch, NULL, NULL, TO_ROOM);
	  act (AT_MAGIC,
	       "As you utter a lengthy incantation, the portal spins into nothingness.",
...
spell_possess (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  if (IS_AFFECTED (victim, AFF_POSSESS)
      || IS_AFFECTED (ch, AFF_CHARM)
      || level < victim->level
      || victim->desc || saves_spell_staff (try, victim) || !chance (ch, 25))
    {
      failed_casting (skill, ch, victim, NULL);
      return rSPELL_FAILED;
    }
      ...
      if (af.duration == 0)
	{
	  ...
	  switch (af.location)
	    ...
	    case APPLY_HIT:
	      ...
	      victim->hit =
		URANGE (0,
			victim->hit + (!IS_NPC (ch)
				       && IS_NPC (victim) ? af.modifier /
				       4 : af.modifier), victim->max_hit);
/*
Blod - August, 2000
...
spell_obj_inv (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  switch (SPELL_ACTION (skill))
	...
	{
	  if (obj->item_type == ITEM_DRINK_CON
	      || obj->item_type == ITEM_FOOD || obj->item_type == ITEM_COOK)
	    {
	      if (obj->item_type == ITEM_COOK && obj->value[2] == 0)
		send_to_char ("It looks undercooked.\n\r", ch);
	      else if (obj->value[3] != 0)
  ...
  if (obj->item_type != ITEM_TREASURE)
      ...
      if (get_curr_int (ch) > 19 && get_curr_wis (ch) > 19)
	{
	  if (number_bits (3) == 1)
		...
		{
		  act (AT_ACTION,
		       "The item is suddenly too heavy for you to carry.", ch,
		       NULL, NULL, TO_CHAR);
...
ch_ret
spell_enchant_armor (int sn, int level, CHAR_DATA * ch, void *vo)
{
  OBJ_DATA *obj = (OBJ_DATA *) vo;
  AFFECT_DATA *paf;
...
ch_ret
spell_death (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  ...
  raw_kill (ch, victim);
  act (AT_MAGIC, "$n's death spell instantly kills $N.", ch, NULL,
       victim, TO_NOTVICT);
  return rVICT_DIED;
}

ch_ret
spell_assassinate (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  OBJ_DATA *obj;
...
ch_ret
spell_grasp_suspiria (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam, l;
  ...
  if (saves_spell_staff (level, victim))
    dam = dam * 3 / 4;
  act (AT_MAGIC, "$n howls with rage as he grabs $N's throat.", ch, NULL,
       victim, TO_NOTVICT);
  return damage (ch, victim, dam, sn);
}
