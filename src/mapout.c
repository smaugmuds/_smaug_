/*
 ...
 [|             /  *********   *        *  *        *   ********    *******   |]
  \\____________\____________________________________________________________//
     ...
     |_____________________________________________________________________|
     |                                                                     |
     | v0.9 6/19/95: Converts an ascii map to rooms.                       |
     | v1.0 7/05/95: Read/write maps to .are files. Efficient storage.     |
     |	             Room qualities based on map code. Can add & remove    |
     |	             rms from a map. (Somewhat) intelligent exit decisions.|
     | v1.1 7/11/95: Various display options. See comments over draw_map.  |
     |_____________________________________________________________________|
    //                                                                     \\
...
/*
 * Local defines.  Undefined at end of file.
 */
...
int add_new_room_to_map (CD * ch, MID * map, int row, int col, int proto_room,
			 char code);
...
void CheckRoom (ROOM_INDEX_DATA * location, int PosX, int PosY,
		char map[50][50], int vnum[50][50], int depth,
		int depthmap[50][50]);
void CheckExitDir (ROOM_INDEX_DATA * location, int PosX, int PosY, int Exd,
		   char map[50][50], int vnum[50][50], int depth,
		   int depthmap[50][50]);
...
char *const map_opts[] = {
  "code", "mobs", "pcs", "objs", "exits", "sector", "light", "indoors",
  "death", "safe", "nosummon", "descr", "descrlines"
};
...
char
get_map_code (RID * room, int mode)
{
...
void
draw_map (CHAR_DATA * ch, char map[50][50])
{
  int x, y;
  char *sect;
 ...
 ******************************************************************/
void
map_stats (CD * ch, int *rooms, int *rows, int *cols)
{
  int row, col, n;
...
void
do_lookmap (CHAR_DATA * ch, char *argument)
{
...
#if 0
void
CheckRoom (ROOM_INDEX_DATA * location, int CX, int CY, char map[50][50],
	   int vnum[50][50], int depth, int depthmap[50][50])
{
  int PosX, PosY;
  ...
  if (PosX <= MapDiameter && PosY <= MapDiameter && PosX > 0 && PosY > 0)
    CheckExitDir (location, PosX, PosY, DIR_NORTH, map, vnum, depth,
		  depthmap);

  PosX = CX;
  PosY = CY + 1;
  if (PosX <= MapDiameter && PosY <= MapDiameter && PosX > 0 && PosY > 0)
    CheckExitDir (location, PosX, PosY, DIR_SOUTH, map, vnum, depth,
		  depthmap);
  ...
  PosY = CY - 1;
  if (PosX <= MapDiameter && PosY <= MapDiameter && PosX > 0 && PosY > 0)
    CheckExitDir (location, PosX, PosY, DIR_NORTHEAST, map, vnum, depth,
		  depthmap);

  PosX = CX - 1;
  PosY = CY - 1;
  if (PosX <= MapDiameter && PosY <= MapDiameter && PosX > 0 && PosY > 0)
    CheckExitDir (location, PosX, PosY, DIR_NORTHWEST, map, vnum, depth,
		  depthmap);

  PosX = CX + 1;
  PosY = CY + 1;
  if (PosX <= MapDiameter && PosY <= MapDiameter && PosX > 0 && PosY > 0)
    CheckExitDir (location, PosX, PosY, DIR_SOUTHEAST, map, vnum, depth,
		  depthmap);

  PosX = CX - 1;
  PosY = CY + 1;
  if (PosX <= MapDiameter && PosY <= MapDiameter && PosX > 0 && PosY > 0)
    CheckExitDir (location, PosX, PosY, DIR_SOUTHWEST, map, vnum, depth,
		  depthmap);
...
void
CheckExitDir (ROOM_INDEX_DATA * location, int X, int Y, int ExD,
	      char map[50][50], int vnum[50][50], int depth,
	      int depthmap[50][50])
{
  EXIT_DATA *xit;
  ...
  for (i = 0; i < 8; ++i)
    {
      ...
      if (xit && xit->to_room && xit->to_room->area == location->area
	  && !IS_SET (xit->exit_info, EX_CLOSED))
	CheckRoom (xit->to_room, CX + nexts[i][1], CY + nexts[i][2], map,
		   vnum, depth + 1, depthmap);
    }
  xREMOVE_BIT (location->room_flags, ROOM_TRACK);
}
#else
void
CheckExit (ROOM_INDEX_DATA * location, int CX, int CY, const int nexts[3],
	   char map[50][50], int vnum[50][50], int depth,
	   int depthmap[50][50])
{
  EXIT_DATA *xit;
  ...
  if (CX <= MapDiameter && CY <= MapDiameter && CX > 0 && CY > 0)
	...
	CheckExit (xit->to_room, CX + nexts[1], CY + nexts[2], nexts, map,
		   vnum, depth + 1, depthmap);
    }
}
...
do_mapout (CD * ch, char *argument)
{
  ...
  if (!str_cmp (arg, "continue"))
      ...
      if (ch->prev_cmd == do_north)
	{
	  ...
	  if (!tmp)
	    {
	      send_to_char
		("No exit to south, or no map in south room. Aborting. \n\r",
		 ch);
	      return;
	    }
      ...
      if (ch->prev_cmd == do_northeast)
	{
	  ...
	  if (!tmp)
	    {
	      send_to_char
		("No exit to southwest, or no map in southwest room. Aborting. \n\r",
		 ch);
	      return;
	    }
      ...
      if (ch->prev_cmd == do_east)
	{
	  ...
	  if (!tmp)
	    {
	      send_to_char
		("No exit to west, or no map in west room. Aborting. \n\r",
		 ch);
	      return;
	    }
      ...
      if (ch->prev_cmd == do_south)
	{
	  ...
	  if (!tmp)
	    {
	      send_to_char
		("No exit to north, or no map in north room. Aborting. \n\r",
		 ch);
	      return;
	    }
      ...
      if (ch->prev_cmd == do_west)
	{
	  ...
	  if (!tmp)
	    {
	      send_to_char
		("No exit to east, or no map in east room. Aborting. \n\r",
		 ch);
	      return;
	    }
      ...
      sprintf (buf,
	       "Your previous command was something I cannot backtrack..\n\r");
      send_to_char (buf, ch);
      return;
    }
  ...
  if (!str_cmp (arg, "addroom"))
      ...
      if ((map_index = get_map_index (mapnum)) == NULL)
	{
	  ...
	  if (map_index == NULL)
	    {
	      send_to_char
		("Could neither find nor make a map index with that number.\n\r",
		 ch);
	      return;
	    }
	}
      ...
      if (this_rm->map != NULL)
	{
	  sprintf (buf, "This room (vnum %d) is already in map %d.\n\r",
		   ch->in_room->vnum, ch->in_room->map->vnum);
	  send_to_char (buf, ch);
	  return;
	}
	...
	{
	  sprintf (buf,
		   "That (x,y) coordinate (%d, %d) is already taken by room %d.\n\r",
  ...
  if (!str_cmp (arg, "removeroom"))
	...
	{
	  sprintf (buf,
		   "Trouble accessing map room(vnum %d), map vnum %d \n\r",
		   ch->in_room->vnum, this_rm->map->vnum);
	  send_to_char (buf, ch);
	  return;
	}
...
int
add_new_room_to_map (CD * ch, MID * map, int row, int col, int proto_room,
		     char code)
{
  int i;
  char buf[MAX_STRING_LENGTH];
 ...
 */
void
map_to_rooms (CD * ch, MID * m_index)
{
  ...
  do
      ...
      if ((map[row][col].index = char_to_number (c)) > -1)
	{
	  ...
	  map[row][col].vnum =
	    add_new_room_to_map (ch, map_index, row, col, proto_vnum, c);
	  ...
	  if (y > 0)
	      ...
	      if ((tvnum = map[y - 1][x].vnum) != 0)
		{
		  ...
		  if (exit_type > -1)
		    {
		      xit =
			make_exit (newrm, get_room_index (tvnum), DIR_NORTH);
	  ...
	  if (x < 79)
	      ...
	      if ((tvnum = map[y][x + 1].vnum) != 0)
		{
		  ...
		  if (exit_type > -1)
		    {
		      xit =
			make_exit (newrm, get_room_index (tvnum), DIR_EAST);
		      xit->keyword = STRALLOC ("");
		      xit->description = STRALLOC ("");
		      xit->key = -1;
		      xit->exit_info = exit_type;
	  ...
	  if (y < 48)
	      ...
	      if ((tvnum = map[y + 1][x].vnum) != 0)
		{
		  ...
		  if (exit_type > -1)
		    {
		      xit =
			make_exit (newrm, get_room_index (tvnum), DIR_SOUTH);
		      xit->keyword = STRALLOC ("");
		      xit->description = STRALLOC ("");
		      xit->key = -1;
		      xit->exit_info = exit_type;
	  ...
	  if (x > 0)
	      ...
	      if ((tvnum = map[y][x - 1].vnum) != 0)
		{
		  ...
		  if (exit_type > -1)
		    {
		      xit =
			make_exit (newrm, get_room_index (tvnum), DIR_WEST);
		      xit->keyword = STRALLOC ("");
		      xit->description = STRALLOC ("");
		      xit->key = -1;
		      xit->exit_info = exit_type;
 ...
 */
int
exit_lookup (int vnum1, int vnum2)
{
  int sect1, sect2;
