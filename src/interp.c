 ...
 */
bool
check_pos (CHAR_DATA * ch, sh_int position)
{
      ...
      while (pw)
	{
	  ...
	  for (cur_imm = pw->imm_name;
		       ...
		       && !str_prefix (pw->player_site, ch->desc->host))))
		{
		  ...
		  if (!(fp = fopen (fname, "a+")))
		    {
		      sprintf (buf, "%s%s", "Write_watch_files: Cannot open ",
			       fname);
		      bug (buf, 0);
		      perror (fname);
		      return;
 ...
 */
char *
case_argument (char *argument, char *arg_first)
{
  char cEnd;
  sh_int count;
...
char *
one_argument2 (register char *argument, char *arg_first)
{
  int count = 0;
  register char ch;
...
void
do_timecmd (CHAR_DATA * ch, char *argument)
{
...
void
start_timer (struct timeval *stime)
{
  if (!stime)
    {
...
time_t
end_timer (struct timeval * stime)
{
  struct timeval etime;
