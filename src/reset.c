/*
 ...
 [|             /  *********   *        *  *        *   ********    *******   |]
  \\____________\____________________________________________________________//
     ...
     |_____________________________________________________________________|
     |                                                                     |
     | This file relies heavily on the fact that your linked lists are     |
     | correct, and that pArea->reset_first is the first reset in pArea.   |
     | Likewise, pArea->reset_last *MUST* be the last reset in pArea.      |
     | Weird and wonderful things will happen if any of your lists are     |
     | messed up, none of them good. The most important are your           |
     | pRoom->contents, pRoom->people, rch->carrying, obj->contains, and   |
     | pArea->reset_first ... pArea->reset_last.  -- Altrag                |
     |_____________________________________________________________________|
    //                                                                     \\
...
RESET_DATA *
find_reset (AREA_DATA * pArea, ROOM_INDEX_DATA * pRoom, int numb)
{
  RESET_DATA *pReset;
  int num = 0;
...
bool
is_room_reset (RESET_DATA * pReset, ROOM_INDEX_DATA * aRoom,
	       AREA_DATA * pArea)
{
  ROOM_INDEX_DATA *pRoom;
  RESET_DATA *reset;
...
ROOM_INDEX_DATA *
find_room (CHAR_DATA * ch, char *argument, ROOM_INDEX_DATA * pRoom)
{
...
void
delete_reset (AREA_DATA * pArea, RESET_DATA * pReset)
{
  RESET_DATA *reset;
...
RESET_DATA *
find_oreset (CHAR_DATA * ch, AREA_DATA * pArea,
...
RESET_DATA *
find_mreset (CHAR_DATA * ch, AREA_DATA * pArea,
	     ROOM_INDEX_DATA * pRoom, char *name)
{
  RESET_DATA *reset;
      ...
      if (!str_prefix (arg, "put"))
	{
	  ...
	  while (reset->next && (reset->next->command == 'H' ||
				 ...
				 (reset->next->command == 'B' &&
				  (reset->next->arg2 & BIT_RESET_TYPE_MASK) ==
				  BIT_RESET_OBJECT && (!reset->next->arg1
						       || reset->next->arg1 ==
						       pReset->arg1))))
	    reset = reset->next;
...
void
do_reset (CHAR_DATA * ch, char *argument)
{
...
void
do_rreset (CHAR_DATA * ch, char *argument)
{
  ROOM_INDEX_DATA *pRoom;
...
void
add_obj_reset (AREA_DATA * pArea, char cm, OBJ_DATA * obj, int v2, int v3)
{
  OBJ_DATA *inobj;
  static int iNest;
  ...
  if ((cm == 'O' || cm == 'P') && obj->pIndexData->vnum == OBJ_VNUM_TRAP)
    {
      if (cm == 'O')
	add_reset (pArea, 'T', obj->value[3], obj->value[1], obj->value[0],
		   v3);
      return;
    }
...
void
instaroom (AREA_DATA * pArea, ROOM_INDEX_DATA * pRoom, bool dodoors)
{
  CHAR_DATA *rch;
  OBJ_DATA *obj;
...
void
wipe_resets (AREA_DATA * pArea, ROOM_INDEX_DATA * pRoom)
{
  RESET_DATA *pReset;
...
void
do_instazone (CHAR_DATA * ch, char *argument)
{
  AREA_DATA *pArea;
  int vnum;
  ROOM_INDEX_DATA *pRoom;
  ...
  for (pReset = pArea->first_reset; pReset; pReset = pReset->next)
	...
	case 'M':
	  if (!(pMobIndex = get_mob_index (pReset->arg1)))
	    {
	      sprintf (buf, "%s Reset_area: 'M': bad mob vnum %d.",
	  ...
	  if (!(pRoomIndex = get_room_index (pReset->arg3)))
	    {
	      sprintf (buf, "%s Reset_area: 'M': bad room vnum %d.",
	...
	case 'E':
	  ...
	  if (mob->pIndexData->pShop)
	    {
	      int olevel =
		UMIN (generate_itemlevel (pArea, pObjIndex), level);
...
*/
		  ...
		  if (!(to_obj = get_obj_type (pObjToIndex)) ||
		      !to_obj->in_room || to_obj->in_room->area != pArea)
		    continue;
		}
	      else
		{
  ...
  for (pReset = pArea->first_reset; pReset; pReset = pReset->next)
      ...
      else			/*Check for 'A', which displays all objects (E, G, O, and P) */
	{
	  if (extraarg == 'A' &&
	      (pReset->command == 'E' ||
	       pReset->command == 'G' ||
	       pReset->command == 'O' || pReset->command == 'P'))
	    show = TRUE;
	}
	...
	case 'M':
	  ...
	  if (!(room = get_room_index (pReset->arg3)))
	    ...
	    rname = room->name;
	  sprintf (pbuf, "%s (%d) -> %s (%d) [%d]", mname, pReset->arg1,
		   rname, pReset->arg3, pReset->arg2);
	  if (!room)
	    mob = NULL;
	...
	case 'E':
	  ...
	  sprintf (pbuf, "%s (%d) -> %s (%s) [%d]", oname, pReset->arg1,
		   mname,
		   (pReset->command ==
		    'G' ? "carry" : wear_locs[pReset->arg3]), pReset->arg2);
	  if (mob && mob->pShop)
	    strcat (buf, " (shop)\n\r");
	  else
	    strcat (buf, "\n\r");
	...
	case 'B':
	  {
		...
		if (ext_bv)
		  ...
		  sprintf (pbuf, "; flags: %s [%d]\n\r",
			   flag_string (pReset->arg3, flagarray),
			   pReset->arg3);
	      }
	    else
...
void
renumber_put_resets (AREA_DATA * pArea)
{
 ...
 */
RESET_DATA *
add_reset (AREA_DATA * tarea, char letter, int extra, int arg1, int arg2,
	   int arg3)
{
  RESET_DATA *pReset;
 ...
 */
RESET_DATA *
place_reset (AREA_DATA * tarea, char letter, int extra, int arg1, int arg2,
	     int arg3)
{
  ...
  if (tarea->first_reset)
	...
	case 'R':
	  ...
	  if (tmp)		/* organize by direction */
	    for (;
		 tmp && tmp->command == letter && tmp->arg1 == tmp->arg1
	...
	case 'O':
	  ...
	  for (; tmp; tmp = tmp->prev)
		...
		if (tmp->arg3 == arg3)
		  for (; tmp; tmp = tmp->prev)
		    if (tmp->command == letter
			&& tmp->arg3 == tmp->arg3 && tmp->arg1 <= arg1)
		      {
			tmp2 = tmp->next;
			break;
		      }
	  ...
	  if ((tmp = tarea->last_mob_reset) != NULL)
	      ...
	      if (tmp->next)
		{
		  ...
		  if (tmp && tmp->command == 'E')
		    {
		      if (letter == 'E')
			for (; tmp && tmp->command == 'E' && tmp->arg1 < arg1;
			     tmp = tmp->next);
		      else
			for (; tmp && tmp->command == 'E'; tmp = tmp->next);
		    }
		  else if (tmp && tmp->command == 'G' && letter == 'G')
		    for (; tmp && tmp->command == 'G' && tmp->arg1 < arg1;
			 tmp = tmp->next);
		  if (tmp)
		    INSERT (pReset, tmp, tarea->first_reset, next, prev);
		  else
		    LINK (pReset, tarea->first_reset, tarea->last_reset, next,
			  prev);
		}
	      else
		LINK (pReset, tarea->first_reset, tarea->last_reset, next,
		...
		if (tmp->next)
		  {
			...
			if (letter == 'P' && tmp->arg3 == arg3)
			  for (;
			       tmp && tmp->command == 'P' && tmp->arg3 == arg3
			       && tmp->arg1 < arg1; tmp = tmp->next);
			else if (letter != 'T')
			  for (;
			       tmp && tmp->command == 'P'
			       && tmp->arg3 == arg3; tmp = tmp->next);
		      }
		    else if (tmp && tmp->command == 'H')
		      {
			if (letter == 'H' && tmp->arg3 == arg3)
			  for (;
			       tmp && tmp->command == 'H' && tmp->arg3 == arg3
			       && tmp->arg1 < arg1; tmp = tmp->next);
			else if (letter != 'H')
			  for (;
			       tmp && tmp->command == 'H'
		      ...
		      for (;
			   tmp && tmp->command == 'T' && tmp->arg3 == arg3
			   && tmp->arg1 < arg1; tmp = tmp->next);
		    if (tmp)
		      INSERT (pReset, tmp, tarea->first_reset, next, prev);
		    else
