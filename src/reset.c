...
/*
 * This file relies heavily on the fact that your linked lists 
 * are correct, and that pArea->reset_first is the first reset 
 * in pArea. Likewise, pArea->reset_last *MUST* be the last reset
 * in pArea.  Weird and wonderful things will happen if any of 
 * your lists are messed up, none of them good.  The most important
 * are your pRoom->contents, pRoom->people, rch->carrying, 
 * obj->contains, and pArea->reset_first ... pArea->reset_last.
 *
 * -- Altrag --
 */
