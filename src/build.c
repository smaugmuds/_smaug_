...
char *const area_flags[] = {
  ...
  "noportalout", "noastral", "nomagic", "r16", "r17", "r18", "r19", "r20",
  "r21", "r22",
...
char *const a_flags[] = {
  ...
  "venomshield", "grapple", "r1", "r2", "r3", "r4", "r5", "r6", "r7", "r8",
  "r9", "r10"
};
...
char *
ext_flag_string (EXT_BV * bitvector, char *const flagarray[])
{
  static char buf[MAX_STRING_LENGTH];
  int x;
...
void
start_editing (CHAR_DATA * ch, char *data)
{
  EDITOR_DATA *edit;
...
do_assist (CHAR_DATA * ch, char *argument)
{
  ...
  if ((victim = get_char_world (ch, arg)))
      ...
      if (!IS_NPC (victim) && IS_SET (victim->pcdata->flags, PCFLAG_DND))
	{
	  ...
	  ch_printf (victim,
		     "Your DND flag just foiled an assist command from %s.\n\r",
		     ch->name);
	  return;
	}
	...
	stop_fighting (ch, TRUE);
      act (AT_ACTION, "$n lifts into the air and is gone.", ch, NULL, NULL,
	   TO_ROOM);
      char_from_room (ch);
      char_to_room (ch, location);
      act (AT_ACTION, "$n appears and floats toward you.", ch, NULL, NULL,
	   TO_ROOM);
      do_look (ch, "auto");
      ...
      for (fch = in_room->first_person; fch; fch = fch_next)
	{
	  ...
	  if (NOT_AUTHED (fch) && ch->in_room->area != fch->in_room->area)
	    continue;
	  if (fch->level > 1
	      && !str_cmp (ch->in_room->area->filename, "SGate.are"))
	    continue;
	  if (fch->master == ch && !IS_NPC (fch))
		...
		stop_fighting (fch, TRUE);
	      act (AT_ACTION, "A magical force pulls you along...", fch, NULL,
		   NULL, TO_CHAR);
	      act (AT_ACTION, "An unseen force pulls $n along behind...", fch,
		   NULL, NULL, TO_ROOM);
	      char_from_room (fch);
	      char_to_room (fch, location);
	      act (AT_ACTION, "$n is pulled along behind...", fch, NULL, NULL,
		   TO_NOTVICT);
	      do_look (fch, "auto");
	    }
	}
...
do_mset (CHAR_DATA * ch, char *argument)
{
  ...
  if (get_trust (ch) < LEVEL_GREATER
      && IS_NPC (victim) && xIS_SET (victim->act, ACT_STATSHIELD))
    {
      send_to_char ("You can't do that!\n\r", ch);
      ch->dest_buf = NULL;
      return;
  ...
  if (!str_cmp (arg2, "affect"))
      ...
      else
	{
	  ...
	  if (loc == APPLY_WEARSPELL && !is_number (arg3))
	    {
	      value =
		bsearch_skill_exact (arg3, gsn_first_spell,
				     gsn_first_skill - 1);
	      if (value == -1)
		{
/*		    printf("%s\n\r", arg3);	*/
...
void
do_rdig (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_rgrid (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
...
char *
sprint_reset (CHAR_DATA * ch, RESET_DATA * pReset, sh_int num, bool rlist)
{
  static char buf[MAX_STRING_LENGTH];
  ...
  switch (pReset->command)
	...
	strcpy (objname, obj->name);
      sprintf (buf, "%2d) %s (%d) -> %s (%s) [%d]\n\r",
	       num,
	       objname,
	       pReset->arg1, mobname, wear_locs[pReset->arg3], pReset->arg2);
      break;
    case 'H':
...
void
do_rloop (CHAR_DATA * ch, char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ROOM_INDEX_DATA *location;
...
void
do_ocreate (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  OBJ_INDEX_DATA *pObjIndex;
...
void
do_mcreate (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
...
void
free_reset (AREA_DATA * are, RESET_DATA * res)
{
...
void
do_aassign (CHAR_DATA * ch, char *argument)
{
  char buf[MAX_STRING_LENGTH];
  AREA_DATA *tarea, *tmp;
...
EXTRA_DESCR_DATA *
SetRExtra (ROOM_INDEX_DATA * room, char *keywords)
{
  EXTRA_DESCR_DATA *ed;
...
bool
DelRExtra (ROOM_INDEX_DATA * room, char *keywords)
{
...
EXTRA_DESCR_DATA *
SetOExtra (OBJ_DATA * obj, char *keywords)
{
  EXTRA_DESCR_DATA *ed;
...
bool
DelOExtra (OBJ_DATA * obj, char *keywords)
{
  EXTRA_DESCR_DATA *rmed;
...
EXTRA_DESCR_DATA *
SetOExtraProto (OBJ_INDEX_DATA * obj, char *keywords)
{
  EXTRA_DESCR_DATA *ed;
...
bool
DelOExtraProto (OBJ_INDEX_DATA * obj, char *keywords)
{
  EXTRA_DESCR_DATA *rmed;
...
void
fold_area (AREA_DATA * tarea, char *filename, bool install)
{
  RESET_DATA *treset;
...
/* Need to convert to new positions correctly on loadup sigh -Shaddai */
      ...
      if (complexmob)
	{
	  ...
	  fprintf (fpout, "%d %d %d %d %d %d %d\n",
		   ...
		   pMobIndex->weight,
		   pMobIndex->speaks,
		   pMobIndex->speaking, pMobIndex->numattacks);
      ...
      if (pObjIndex->layers || ((AREA_VERSION_WRITE > 1) && pObjIndex->level))
	{
	  if (AREA_VERSION_WRITE > 1)
	    fprintf (fpout, "%d %s %d %d %d\n", pObjIndex->item_type,
		     ...
		     pObjIndex->layers, pObjIndex->level);
	  else
      ...
      for (xit = room->first_exit; xit; xit = xit->next)
	{
	  ...
	  if (xit->distance > 1 || xit->pull)
	    fprintf (fpout, "%d %d %d %d %d %d\n",
		     ...
		     xit->key,
		     xit->vnum, xit->distance, xit->pulltype, xit->pull);
	  else
	    fprintf (fpout, "%d %d %d\n", xit->exit_info & ~EX_BASHED,
		     xit->key, xit->vnum);
	}
...
void
do_loadarea (CHAR_DATA * ch, char *argument)
{
  AREA_DATA *tarea;
  char filename[256];
  int tmp;
 ...
 */
void
do_unfoldarea (CHAR_DATA * ch, char *argument)
{
...
void
do_foldarea (CHAR_DATA * ch, char *argument)
{
  AREA_DATA *tarea;
 ...
 */
void
do_installarea (CHAR_DATA * ch, char *argument)
{
  AREA_DATA *tarea;
  char arg[MAX_INPUT_LENGTH];
  char buf[MAX_STRING_LENGTH];
...
void
add_reset_nested (AREA_DATA * tarea, OBJ_DATA * obj)
{
  int limit;
 ...
 */
RESET_DATA *
parse_reset (AREA_DATA * tarea, char *argument, CHAR_DATA * ch)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
...
void
do_aset (CHAR_DATA * ch, char *argument)
{
  AREA_DATA *tarea;
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
...
void
do_opedit (CHAR_DATA * ch, char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
...
void
do_rpedit (CHAR_DATA * ch, char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
...
void
do_rdelete (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  ROOM_INDEX_DATA *location;
...
void
do_odelete (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_INDEX_DATA *obj;
  OBJ_DATA *temp;
...
void
do_mdelete (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  MOB_INDEX_DATA *mob;
  CHAR_DATA *temp;
...
void
do_opcopy (CHAR_DATA * ch, char *argument)
{
...
void
do_mpcopy (CHAR_DATA * ch, char *argument)
{
...
void
do_rpcopy (CHAR_DATA * ch, char *argument)
{
 ...
 */
void
do_climate (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  AREA_DATA *area;
  ...
  /* add or remove neighboring weather systems */
      ...
      if (neigh)
	{
	  ...
	  if (tarea)
		   ...
		   tneigh; tneigh = tneigh->next)
		{
	      ...
	      UNLINK (tneigh,
		      ...
		      tarea->weather->last_neighbor, next, prev);
	      STRFREE (tneigh->name);
	      DISPOSE (tneigh);
	    }
	  ...
	  UNLINK (neigh,
		  ...
		  area->weather->last_neighbor, next, prev);
...
void
do_findexit (CHAR_DATA * ch, char *argument)
{
  ROOM_INDEX_DATA *room;
  EXIT_DATA *pexit;
	...
	for (room = room_index_hash[cou]; room; room = room->next)
	  {
		...
		&& room->vnum <= h_range && room->first_exit)
	      {
	...
	for (room = room_index_hash[cou]; room; room = room->next)
	  {
	    ...
	    for (exit = room->first_exit; exit; exit = exit->next)
		  ...
		  || (reverse
		      && room->area != area
		      && exit->to_room && exit->to_room->area == area))
		{
		  if (!efound)
