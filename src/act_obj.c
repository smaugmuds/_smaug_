...
do_connect (CHAR_DATA * ch, char *argument)
{
  ...
  if (first_ob->item_type != ITEM_PIECE || second_ob->item_type != ITEM_PIECE)
    {
      send_to_char
	("You stare at them for a moment, but these items clearly don't go together.\n\r",
	 ch);
      return;
    }
  ...
  if ((first_ob->value[0] == second_ob->pIndexData->vnum)
    ...
    /* good connection  */
    {
      ...
      act (AT_ACTION,
	   "$n cobbles some objects together....\r\nsuddenly they snap together, creating $p!",
	   ch, new_ob, NULL, TO_ROOM);
      act (AT_ACTION,
	   "You cobble the objects together....\r\nsuddenly they snap together, creating $p!",
    ...
    {
      act (AT_ACTION,
	   "$n jiggles some objects against each other, but can't seem to make them connect.",
	   ch, NULL, NULL, TO_ROOM);
      act (AT_ACTION,
	   "You try to fit them together every which way, but they just don't want to connect.",
...
do_get (CHAR_DATA * ch, char *argument)
{
  ...
  if (ch->carry_number < 0 || ch->carry_weight < 0)
	...
	("Uh oh ... better contact an immortal about your number or weight of items carried.\n\r",
	 ch);
      sprintf (buf, "%s has negative carry_number or carry_weight!",
	       ch->name);
      log_string (buf);
      return;
    }
...
/* blodkai arg */
      ...
      else
	{
	  ...
	  if (IS_OBJ_STAT (container, ITEM_CLANCORPSE)
	      && !IS_IMMORTAL (ch)
	      && !IS_NPC (ch) && str_cmp (ch->name, container->name + 7))
	    {
	  ...
	  if (!found)
	      ...
	      if (fAll)
		{
		  if (container->item_type == ITEM_KEYRING
		      && !IS_OBJ_STAT (container, ITEM_COVERING))
		    act (AT_PLAIN, "The $T holds no keys.", ch, NULL, arg2,
			 TO_CHAR);
		  else
		    act (AT_PLAIN, IS_OBJ_STAT (container, ITEM_COVERING) ?
...
// Fix end
	  ...
	  && container->in_room->max_weight <
	  get_real_obj_weight (obj) / obj->count + container->in_room->weight)
	{
	  send_to_char ("It won't fit.\n\r", ch);
	  return;
	}
...
do_drop (CHAR_DATA * ch, char *argument)
{
  ...
  if (number > 0)
      ...
      if (!str_cmp (arg, "coins") || !str_cmp (arg, "coin"))
	{
	  ...
	  if (ch->level < 3 && ch->gold <= 50000)
	    {
	      send_to_char
		("You should gain more experience before throwing away gold.\n\r",
		 ch);
	      return;
	    }
  ...
  if (number <= 1 && str_cmp (arg, "all") && str_prefix ("all.", arg))
      ...
      if (!can_drop_obj (ch, obj))
	{
	  ...
	  get_real_obj_weight (obj) / obj->count + ch->in_room->weight)
	{
	  send_to_char
	    ("There is not enough room on the ground for that.\n\r", ch);
	  return;
	}
...
do_give (CHAR_DATA * ch, char *argument)
{
  ...
  if (IS_IMMORTAL (ch) || (IS_NPC (ch) && xIS_SET (ch->act, ACT_IMMORTAL)))
    immgive = TRUE;
  ...
  if (!can_see_obj (victim, obj))
	...
	{
	  act (AT_PLAIN, "$N cannot see this item - placed in inventory.", ch,
	       NULL, victim, TO_CHAR);
	}
    }
...
damage_obj (OBJ_DATA * obj)
{
  ...
  if (objcode == rOBJ_SCRAPPED && !IS_NPC (ch))
    {
      sprintf (buf, "%s scrapped %s (vnum: %d)", ch->name, obj->short_descr,
	       obj->pIndexData->vnum);
      log_string (buf);
    }
  return objcode;
 ...
 */
bool
can_layer (CHAR_DATA * ch, OBJ_DATA * obj, sh_int wear_loc)
{
  OBJ_DATA *otmp;
  sh_int bitlayers = 0;
 ...
 */
void
wear_obj (CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, sh_int wear_bit)
{
  char buf[MAX_STRING_LENGTH];
  ...
  if (obj->owner[0] != '\0' && !IS_NPC (ch) && str_cmp (obj->owner, ch->name))
    {
      ...
      act (AT_MAGIC, "You are not the owner of that item.", ch, NULL, NULL,
	   TO_CHAR);
      act (AT_ACTION, "$n tries to use $p, but is not the owner.", ch, obj,
	   NULL, TO_ROOM);
      act (AT_MAGIC, "You feel sick as a wrenching pain overcomes your body.",
	   ch, NULL, NULL, TO_CHAR);
      act (AT_ACTION, "$n doubles over in pain and drops $p.", ch, obj, NULL,
	   TO_ROOM);
      if (obj->carried_by)
	obj_from_char (obj);
...
do_sacrifice (CHAR_DATA * ch, char *argument)
{
  ...
  if (obj->item_type == ITEM_PAPER)
    {
      sprintf (log_buf, "%s sacrificed note: %s", ch->name, obj->short_descr);
 ...
 */
void
save_storeroom (CHAR_DATA * ch, char *vnum)
{
  FILE *fp;
  char filename[256];
  sh_int templvl;
...
do_auction (CHAR_DATA * ch, char *argument)
{
  ...
  if ((time_info.hour > 18 || time_info.hour < 9)
      && auction->item == NULL && !IS_IMMORTAL (ch))
    {
      send_to_char
...
/* finally... */
  ...
  if (obj->item_type != obj->pIndexData->item_type)
    {
      send_to_char
	("This object is too modified to auction.  Please contact an immortal.\n\r",
	 ch);
      return;
    }
...
void
do_findnote (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *obj;
...
OBJ_DATA *
recursive_note_find (OBJ_DATA * obj, char *argument)
{
