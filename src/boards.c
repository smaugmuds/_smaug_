  ...
  if ((fp = fopen (filename, "w")) == NULL)
      ...
      for (pnote = board->first_note; pnote; pnote = pnote->next)
	{
	  fprintf (fp,
		   ...
		   pnote->abstentions, pnote->yestally, pnote->notally,
		   pnote->abstaintally, pnote->no_remove, pnote->text);
	}
      fclose (fp);
    }
...
void
note_remove (CHAR_DATA * ch, BOARD_DATA * board, NOTE_DATA * pnote)
{
...
void
do_noteroom (CHAR_DATA * ch, char *argument)
{
  BOARD_DATA *board;
...
void
do_mailroom (CHAR_DATA * ch, char *argument)
{
  BOARD_DATA *board;
  char arg[MAX_STRING_LENGTH];
  char arg_passed[MAX_STRING_LENGTH];
...
void
do_note (CHAR_DATA * ch, char *arg_passed, bool IS_MAIL)
{
...
*/
  ...
  if (!str_cmp (arg, "list") || !str_cmp (arg, "mine"))
      ...
      else
	{
	  ...
	  for (pnote = board->first_note; pnote; pnote = pnote->next)
	    if (is_note_to (ch, pnote)
		|| (!mine && get_trust (ch) >= sysdata.read_all_mail))
		  ...
		  if (pnote->yesvotes[0] != '\0' || pnote->novotes[0] != '\0'
			...
			("------------------------------------------------------------\n\r",
			 ch);
			  ...
			  || (board->extra_ballots[0] != '\0'
			      && is_name (ch->name, board->extra_ballots)))
			pager_printf (ch,
				      "Votes:\n\rYes:  %s\n\rNo:  %s\n\rAbstain: %s\n\r",
  ...
  if (!str_cmp (arg, "vote"))
      ...
      if (!str_cmp (arg_passed, "ballot"))
	{
	  ...
	  act (AT_ACTION, "$n opens a ballot on a note.", ch, NULL, NULL,
	       TO_CANSEE);
      ...
      if (!str_cmp (arg_passed, "close"))
	{
	  ...
	  pnote->voting =
	    (pnote->voting == VOTE_BALLOT ? VOTE_BCLOSED : VOTE_CLOSED);
      ...
      if (!str_cmp (arg_passed, "abstain"))
	{
	  ...
	  pnote->abstaintally =
	    ((pnote->abstaintally) ? pnote->abstaintally + 1 : 1);
	  act (AT_ACTION, "$n votes on a note.", ch, NULL, NULL, TO_CANSEE);
...
/*	char *pn;*/
  ...
  if (!str_cmp (arg, "reply"))
	...
	{
	  send_to_char
	    ("You cannot read the notes on this board to reply to.\n\r", ch);
	  return;
	}
  ...
  if (!str_cmp (arg, "remove")
      ...
      for (pnote = board->first_note; pnote; pnote = pnote->next)
	{
	  ...
	  if ((is_note_to (ch, pnote)
		...
		{
		  send_to_char
		    ("Notes addressed to 'all' can only be taken by the author.\n\r",
		     ch);
		  return;
		}
...
BOARD_DATA *
read_board (char *boardfile, FILE * fp)
{
  BOARD_DATA *board;
  char *word;
  char buf[MAX_STRING_LENGTH];
  ...
  for (;;)
	...
	case 'E':
	  ...
	  KEY ("Extra_removers", board->extra_removers,
	       fread_string_nohash (fp));
	  KEY ("Extra_ballots", board->extra_ballots,
	       fread_string_nohash (fp));
	  if (!str_cmp (word, "End"))
	    {
	      board->num_posts = 0;
...
NOTE_DATA *
read_note (char *notefile, FILE * fp)
{
  NOTE_DATA *pnote;
  char *word;
...
void
do_makeboard (CHAR_DATA * ch, char *argument)
{
  BOARD_DATA *board;
...
void
do_bset (CHAR_DATA * ch, char *argument)
{
  BOARD_DATA *board;
  bool found;
  ...
  if (arg1[0] == '\0' || arg2[0] == '\0')
    {
      ...
      send_to_char
	("  read_group post_group extra_readers extra_removers extra_ballots\n\r",
	 ch);
...
void
do_bstat (CHAR_DATA * ch, char *argument)
{
  BOARD_DATA *board;
  bool found;
  char arg[MAX_INPUT_LENGTH];
...
void
do_journal (CHAR_DATA * ch, char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char buf[MAX_STRING_LENGTH];
  ...
  switch (ch->substate)
	...
	{
	  bug ("do_journal: Player not holding journal. (Player: %s)",
	       ch->name);
	  stop_editing (ch);
	  return;
	}
 ...
 */
  ...
  if (!str_cmp (arg1, "write"))
	...
	{
	  send_to_char
	    ("You must be holding a journal in order to write in it.\n\r",
	     ch);
	  return;
	}
	...
	{
	  send_to_char
	    ("There are no pages in this journal. Seek an immortal for assistance.\n\r",
	     ch);
	  return;
	}
/* Force a max value of 50 */
      if (journal->value[0] > 50)
	{
	  journal->value[0] = 50;
	  bug
	    ("do_journal: Journal size greater than 50 pages! Resetting to 50 pages. (Player: %s)",
	     ch->name);
	}
...
/* Size option, returns how many pages are in the journal */
  ...
  if (!str_cmp (arg1, "size"))
	...
	{
	  send_to_char
	    ("You must be holding a journal in order to check it's size.\n\r",
	     ch);
	  return;
	}
      if (journal->value[0] < 1)
 ...
 */
  if (!str_cmp (arg1, "read"))
	...
	{
	  send_to_char
	    ("You must be holding a journal in order to read it.\n\r", ch);
	  return;
	}
      if (journal->value[0] > 50)
	{
