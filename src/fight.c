...
is_wielding_poisoned (CHAR_DATA * ch)
{
  ...
  if ((obj = get_eq_char (ch, WEAR_WIELD)) != NULL
      && used_weapon == obj && IS_OBJ_STAT (obj, ITEM_POISONED))
    return TRUE;
...
multi_hit (CHAR_DATA * ch, CHAR_DATA * victim, int dt)
{
  ...
  if (!IS_NPC (ch) && ch->stance != STANCE_MONKEY &&
	...
	for (chance = 0; chance < stance_index[ch->stance].num_attacks;
	     chance++)
	  {
	    retcode = one_hit (ch, victim, dt);
	    if (retcode != rNONE || who_fighting (ch) != victim)
 ...
 */
sh_int
ris_damage (CHAR_DATA * ch, sh_int dam, int ris)
{
  sh_int modifier;
...
damage (CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt)
{
  ...
  if (victim != ch)
	...
	if (is_same_group (victim, gch) && !IS_NPC (gch)
       ...
       */
      if ((bnopc > 0 && anopc > 0)
	  || (bnopc > 0 && !IS_NPC (ch)) || (anopc > 0 && !IS_NPC (victim)))
	{
	  /*
      ...
      if (ch->stance != STANCE_MONKEY && victim->stance != STANCE_MONKEY)
	{
	  if (ch->stance > STANCE_NONE
	      && stance_index[ch->stance].dam_done > 0)
	    {
	      float temp_dam;
	      if (IS_NPC (ch))
		{
	  ...
	  if (victim->stance > STANCE_NONE
	      && stance_index[victim->stance].dam_taken > 0)
	    {
	      float temp_dam;
	      if (IS_NPC (victim))
		{
		  ...
		  temp_dam =
		    (victim->pIndexData->stances[victim->stance] / 200.0);
		  if (temp_dam < .5)
		    temp_dam = .5;
		}
	      else
		{
		  dam *= (stance_index[victim->stance].dam_taken / 100.0);
		  temp_dam =
		    (victim->pcdata->stances[victim->stance] / 200.0);
		  if (temp_dam < .5)
		    temp_dam = .5;
		}
	      dam /= temp_dam;
      ...
      if (dt >= TYPE_HIT && ch->in_room == victim->in_room)
	{
	  ...
	  if (IS_NPC (ch)
	      ...
	      && ch->level > 5 && number_percent () < ch->level / 2)
	    trip (ch, victim);
...
/*  ch_printf( ch, "(%d damage)\n\r", dam ); */
  ...
  if (dam > 0 && dt > TYPE_HIT
      ...
      && !saves_poison_death (ch->level, victim))
    {
      ...
      victim->mental_state =
	URANGE (20, ch->mental_state + (IS_PKILL (ch) ? 1 : 2), 100);
    }
  ...
  if (victim->position == POS_DEAD)
      ...
      if (!npcvict)
	{
	  ...
	  sprintf (log_buf, "%s (%d) killed by %s at %d", victim->name,
	  ...
	  if (!IS_NPC (ch) && !IS_IMMORTAL (ch) && ch->pcdata->clan
	      ...
	      && ch->pcdata->clan->clan_type != CLAN_GUILD && victim != ch)
	    {
	      ...
	      sprintf (log_buf, "&P(%2d) %-12s &wvs &P(%2d) %s &P%s ... &w%s",
		       ...
		       !CAN_PKILL (victim) ? "&W<Peaceful>" : victim->
		       pcdata->clan ? victim->pcdata->
	  ...
	  if (!IS_NPC (victim) && !IS_IMMORTAL (victim)
	      ...
	      && victim->pcdata->clan->clan_type != CLAN_GUILD && ch != victim
	      && !IS_NPC (ch))
	    {
	      sprintf (filename2, "%s%s.defeats", CLAN_DIR,
		       victim->pcdata->clan->name);
	      sprintf (log_buf,
		       "&P(%2d) %-12s &wdefeated by &P(%2d) %s &P%s ... &w%s",
		       victim->level, victim->name, ch->level,
		       !CAN_PKILL (ch) ? "&W<Peaceful>" : ch->
		       pcdata->clan ? ch->pcdata->clan->
		       badge : "&P(&WUnclanned&P)", ch->name,
...
is_safe (CHAR_DATA * ch, CHAR_DATA * victim, bool show_messg)
{
  ...
  if (!IS_NPC (ch) && !IS_NPC (victim)
      && ch != victim && IS_SET (victim->in_room->area->flags, AFLAG_NOPKILL))
    {
      if (show_messg)
	{
...
check_killer (CHAR_DATA * ch, CHAR_DATA * victim)
{
  ...
  if (IS_SET (ch->in_room->area->flags, AFLAG_FREEKILL)
      ...
      else
	{
	  ...
	  || ch->pcdata->honour == 20
	  || ch->pcdata->honour == 40
	  || ch->pcdata->honour == 65 || ch->pcdata->honour == 100)
	send_to_char
	  ("Congratulations. You have obtained a new rank of honour. \n\r",
	   ch);
    }
  ...
  if (victim->hit > 0)
	  ...
	  && !IS_AFFECTED (victim, AFF_PARALYSIS))
	{
	  ...
	  if (((victim->max_hit / 4) > victim->hit) || !gm_stance)
	    {
	      act (AT_ACTION, "$n clambers back to $s feet.", victim, NULL,
		   NULL, TO_ROOM);
	      act (AT_ACTION, "You clamber back to your feet.", victim, NULL,
		   NULL, TO_CHAR);
	    }
	  else
	    {
	      act (AT_ACTION, "$n flips back up to $s feet.", victim, NULL,
		   NULL, TO_ROOM);
	      act (AT_ACTION, "You flip back up to your feet.", victim, NULL,
		   NULL, TO_CHAR);
	    }
	}
      if (IS_AFFECTED (victim, AFF_PARALYSIS))
...
death_cry (CHAR_DATA * ch)
{
  ...
  switch (number_range (0, 5))
    ...
    case 3:
      ...
      break;
    case 4:
      msg = "$n gasps $s last breath and blood spurts out of $s "
	"mouth and ears.";
      break;
    case 5:
 ...
 */
void
new_dam_message (CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt,
		 OBJ_DATA * obj)
{
  ...
  if (!IS_NPC (victim) && !IS_NPC (ch))
    {
      if ((!IS_SET (victim->pcdata->flags, PCFLAG_DEADLY)
	   ...
	   || !IS_SET (ch->pcdata->flags, PCFLAG_DEADLY))
	  && !in_arena (ch)
	  && ch != victim && !(IS_IMMORTAL (ch) && IS_IMMORTAL (victim)))
	{
	  if (IS_NPC (ch))
	  ...
	  sprintf (log_buf,
		   "&p%s on %s%s in &G&R***&rILLEGAL PKILL&G&R*** &pattempt at %d",
		   (lastplayercmd),
  ...
  for (attempt = 0; attempt < 8; attempt++)
      ...
      if (!IS_NPC (ch))
	{
	  ...
	  act (AT_FLEE, "You flee head over heels from combat!", ch, NULL,
	       NULL, TO_CHAR);
	  los =
	    (exp_level (ch, ch->level + 1) -
	     exp_level (ch, ch->level)) * 0.02;
	  if (ch->level < 50)
	    {
	      if (!IS_PKILL (ch))
		{
