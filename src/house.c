/*
 ...
 [|             /  *********   *        *  *        *   ********    *******   |]
  \\____________\____________________________________________________________//
     ...
     |_____________________________________________________________________|
     |                                                                     |
     |                Author : Senir  (oldgaijin@yahoo.com)                |
...
bool
in_same_house (CHAR_DATA * ch, CHAR_DATA * vch)
{
  HOME_DATA *home;
  int x, y;
...
void
do_house (CHAR_DATA * ch, char *argument)
{
  bool apt = FALSE;
  ...
  if (!argument || argument[0] == '\0')
      ...
      if (IS_IMMORTAL (ch))
	{
	  ...
	  send_to_char
	    (" set <character name> addroom <current house vnum> <direction>\n\r",
	     ch);
  ...
  if (!str_cmp (arg, "givekey"))
      ...
      if (get_trust (ch) < LEVEL_DEMI)
	{
	  if (ch->gold >= DEFAULT_MOB_PRICE)
	    ...
	    {
	      send_to_char
		("You do not have the necessary gold to make a key.\n\r", ch);
	      return;
	    }
	}
      else
	{
      ...
      if (!give_key
	  (ch, homedata->vnum[0], homedata->name, homedata->apartment))
	{
  ...
  if (homedata)
	...
	if (homedata->vnum[i] == ch->in_room->vnum)
      ...
      if (!location)
	if (!IS_IMMORTAL (ch)
	    || (location = get_room_index (homedata->vnum[0])) == NULL)
	  {
	    send_to_char
	      ("You must be in your house to make changes to it.\n\r", ch);
	    return;
	  }
      ...
      if (!str_cmp (arg, "name"))
	{
	  ...
	  sprintf (buf, "%s: house %s %s", capitalize (ch->name), arg,
		   argument);
	  log_string (buf);
	  return;
	}
      ...
      if (!str_cmp (arg, "addroom") && !apt)
	{
	  if (ch->gold < ADDITIONAL_ROOM_COST)
	    {
	      send_to_char
		("You do not have enough money for an additional room.\n\r",
		 ch);
	      return;
	    }
	  ...
	  if (i == MAX_HOUSE_ROOMS)
	    {
	      send_to_char
		("You currently have the maximum number of additional rooms.\n\r",
		 ch);
	      return;
	    }
	  ...
	  if (!add_room (homedata, location, argument))
	    {
	      send_to_char
		("An exit could not be created.  You may need to contact an immortal.\n\r",
		 ch);
	      return;
	    }
  ...
  if (!str_cmp (arg, "set"))
	...
	{
	  send_to_char
	    ("Syntax: house set <character name> <vnum> (apartment)\n\r", ch);
	  send_to_char
	    ("        house set <character name> addroom <current vnum> <direction>\n\r",
	     ch);
	  return;
	}
      ...
      if (!str_cmp (arg3, "addroom"))
	{
	  ...
	  if (!argument || argument[0] == '\0'
	    ...
	    {
	      send_to_char
		("Syntax: house set <character name> addroom <current vnum> <direction>\n\r",
		 ch);
	      return;
	    }
	  ...
	  if (i == MAX_HOUSE_ROOMS)
	    {
	      send_to_char
		("That character already has the maximum number of additional rooms.\n\r",
		 ch);
	      return;
	    }
  ...
  if (!str_cmp (arg, "remove"))
	...
	{
	  send_to_char
	    ("That character does not appear to have a residence.\n\r", ch);
	  return;
	}
...
void
do_gohome (CHAR_DATA * ch, char *argument)
{
  HOME_DATA *home;
  ...
  sprintf (buf,
	   "As a vortex of luminescent light forms, %s slides gracefully into their home.\n\r",
	   ch->name);
  act (AT_GREY, buf, ch, NULL, ch, TO_ROOM);
...
void
do_residence (CHAR_DATA * ch, char *argument)
{
  HOME_DATA *home;
  ROOM_INDEX_DATA *room;
  sh_int i, z = 1;
  ...
  for (home = first_home; home; home = home->next)
      ...
      if ((room = get_room_index (home->vnum[0])) == NULL)
	{
	  ...
	  sprintf (buf, "Residence: NULL start room vnum - C:%s V:%d",
		   home->name, home->vnum[0]);
	  bug (buf, 0);
	...
	if (home->vnum[i] <= 0)
      ...
      ch_printf_color (ch,
		       "&w%3d&G| &w%-15.15s &G| &w%-28.28s &G|  &w%s &G| &w%5d &G| &w%10d &G|\n\r",
		       z, home->name, room->area->name,
		       home->apartment ? "A" : "H", i, home->vnum[0]);
...
void
do_accessories (CHAR_DATA * ch, char *argument)
{
  ...
  if (!str_cmp (arg, "list"))
	...
	{
	  send_to_char_color
	    ("&g## &G|        &gAccessory Name        &G|   &gPrice    &G|\n\r",
	     ch);
	  send_to_char_color
	    ("&G------------------------------------------------\n\r", ch);
	}
	...
	{
	  send_to_char_color
	    ("&g## &G|        &gAccessory Name        &G|   &gPrice    &G|    &gVnum    &G|\n\r",
	     ch);
	  send_to_char_color
	    ("&G-------------------------------------------------------------\n\r",
	     ch);
      ...
      for (acc = first_accessory; acc; acc = acc->next)
	{
	  if (!acc->mob)
		...
		{
		  sprintf (buf,
			   "Accessories list: object vnum %d does not exist.",
			   acc->vnum);
		  bug (buf, 0);
		  ch_printf_color (ch,
				   "&g%3d&G| &RAccessory currently unavailable.\n\r",
				   i);
		  i++;
		  continue;
		...
		{
		  sprintf (buf,
			   "Accessories list: mob vnum %d does not exist.",
			   acc->vnum);
		  bug (buf, 0);
		  ch_printf_color (ch,
				   "&g%3d&G| &RAccessory currently unavailable.\n\r",
	  ...
	  if (!IS_IMMORTAL (ch))
	    ...
	    {
	      ch_printf_color (ch,
			       "&g%3d&G| &g%-28.28s &G| &g%10d &G| &g%10d &G|\n\r",
  ...
  if (str_cmp (arg, "show"))
    {
      sprintf (buf, "%s: accessories %s %s", capitalize (ch->name), arg,
	       argument);
      log_string (buf);
    }
  ...
  if (!str_cmp (arg, "buy"))
      ...
      if (acc->mob && (mobindex = get_mob_index (acc->vnum)) != NULL)
	{
	  ...
	  add_reset (location->area, 'M', 1, mobindex->vnum, mobindex->count,
		     location->vnum);
	  save_residence (location);
	}
      ...
      else
	{
	  ...
	  sprintf (buf,
		   "Accessory buy: accessory does not exist - V: %d  #: %d",
		   acc->vnum, i);
	  bug (buf, 0);
	  return;
	}
  ...
  if (!str_cmp (arg, "show"))
      ...
      if (acc->mob && (mob = get_mob_index (acc->vnum)) != NULL)
	{
	  ...
	  ch_printf_color (ch, "&gAverage AC: &G%d\n\r",
			   (mob->
			    ac ? mob->ac : interpolate (mob->level, 100,
							-100)));
	...
	{
	  sprintf (buf,
		   "Accessory show: accessory does not exist - V: %d  #: %d",
		   acc->vnum, i);
	  bug (buf, 0);
...
void
do_homebuy (CHAR_DATA * ch, char *argument)
{
  HOMEBUY_DATA *homeb;
  HOME_DATA *home;
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!argument || argument[0] == '\0' || !str_cmp (argument, "list"))
      ...
      if (!first_homebuy)
	{
      ...
      send_to_char_color
	("&c##&C| &cResidence Name       &C| &cHousing Area   &C|&cCurr. Bid &C| &cBid Inc. &C|&cRemaining Time&C|\n\r",
	 ch);
      send_to_char_color
	("&C--------------------------------------------------------------------------------\n\r",
	 ch);
      ...
      for (homeb = first_homebuy; homeb; homeb = homeb->next)
	{
	  ...
	  if ((location = get_room_index (homeb->vnum)) == NULL)
	    {
	      ...
	      sprintf (buf,
		       "Homebuy list: location does not exist.  -V:%d  #:%d",
		       homeb->vnum, i);
	      bug (buf, 0);
	      ch_printf_color (ch,
			       "&c%2d&C| &RThis residence not currently available.",
			       i);
	      continue;
	    }
	  ...
	  ch_printf_color (ch,
			   "&c%2d&C|&c%-22.22s&C|&c%-16.16s&C|&c%10d&C|&c%10d&C|&c%2d &CDays &c%2d &CHrs|\n\r",
			   i, name, area, homeb->bid, bidinc, days,
  ...
  if (!str_cmp (arg, "syntax"))
      ...
      if (IS_IMMORTAL (ch))
	{
	  ...
	  send_to_char (" bidincrement <# of auction item>  <percent>\n\r",
			ch);
	  send_to_char
	    (" timeremainder <# of auction item> <days> (<hours>)\n\r", ch);
  ...
  if (!str_cmp (arg, "add") && get_trust (ch) >= LEVEL_GREATER)
      ...
      if ((location = get_room_index (vnum)) != NULL)
	{
	  if (xIS_SET (location->room_flags, ROOM_HOUSE))
	    {
	      send_to_char
		("That room is already a residence.  Use \"sellhouse\" or first remove that player's house.\n\r",
		 ch);
	      return;
	    }
	}
  ...
  if (!str_cmp (arg, "show"))
      ...
      if ((location = get_room_index (homeb->vnum)) == NULL)
	{
	  ...
	  sprintf (buf,
		   "Homebuy show: location does not exist. -V: %d, #: %d",
		   homeb->vnum, i);
	  bug (buf, 0);
	  send_to_char
	    ("That residence does not exist.  Please contact an immortal.\n\r",
	     ch);
	  return;
	}
  ...
  if (!str_cmp (arg, "bid"))
	...
	if (!str_cmp (home->name, ch->name))
	  {
	    send_to_char
	      ("You already own a home. You must sell that one first.\n\r",
	       ch);
	    return;
	  }
	...
	{
	  send_to_char
	    ("You must bid more than the current bid plus the bid increment.\n\r",
	     ch);
	  return;
	}
...
void
do_sellhouse (CHAR_DATA * ch, char *argument)
{
  HOME_DATA *home;
  int bid;
  ...
  if (!argument || argument[0] == '\0')
    {
      ...
      send_to_char
	("    Places your residence on auction with default starting bid.\n\r",
	 ch);
      send_to_char ("  Sellhouse <amount>\n\r", ch);
      send_to_char
	("   Places your residence on auction with the given amount as starting bid.\n\r",
	 ch);
      return;
    }
  ...
  if (str_cmp (argument, "yes"))
	...
	{
	  send_to_char
	    ("Bid amount must be equal to or greater than the minimum bid for the residence type.\n\r",
	     ch);
	  return;
	}
    }
  ...
  send_to_char
    ("You have successfully placed your residence on auction for the given starting bid.\n\r"
...
void
save_residence (ROOM_INDEX_DATA * location)
{
...
bool
set_house (CHAR_DATA * ch, int vnum, bool apartment)
{
  ...
  for (pexit = location->first_exit; pexit; pexit = pexit->next)
      ...
      if (pexit->rexit)
	{
	  ...
	  if ((revroom = get_room_index (pexit->vnum)))
	    {
	      add_reset (revroom->area, 'D', 0, revroom->vnum,
			 pexit->rexit->vdir, 2);
...
bool
add_room (HOME_DATA * home, ROOM_INDEX_DATA * location, char *argument)
{
  ...
  if ((pexit = make_exit (location, addloc, get_dir (argument))) == NULL
      || (rexit =
	  make_exit (addloc, location, rev_dir[get_dir (argument)])) == NULL)
    {
      delete_room (addloc);
      bug
	("add_room: exits not able to be created for room.  location deleted.",
	 0);
      return FALSE;
    }
...
bool
give_key (CHAR_DATA * ch, int vnum, char *name, bool apt)
{
  ...
  if ((keyindex = get_obj_index (vnum)) == NULL)
      ...
      if (get_trust (ch) >= LEVEL_GREATER)
	{
	  ...
	  send_to_char
	    ("Key previously not created.  You must fold houses.are to save this key.\n\r",
	     ch);
	}
      else
	return FALSE;
...
void
fwrite_house (HOME_DATA * homedata)
{
  FILE *fpout;
  ROOM_INDEX_DATA *location = NULL;
  OBJ_DATA *obj = NULL;
...
load_house_file (char *name)
{
  ...
  for (;;)
      ...
      if (!str_cmp (word, "HOME"))
	{
	  ...
	  if (!pRoom)
	    {
	      sprintf (buf, "Load_house_file: bad house vnum in %s.  Vnum %d",
		       filename, vnum);
	      boot_log (buf);
	      return FALSE;
	    }
	...
	{
	  sprintf (buf, "Load_house_file: bad section: %s in file %s.", word,
		   filename);
	  boot_log (buf);
	  break;
	}
...
bool
remove_homebuy (HOMEBUY_DATA * tmphome)
{
  if (!tmphome)
    return FALSE;
...
load_homebuy ()
{
  ...
  if ((fp = fopen (HOMEBUY_FILE, "r")) == NULL)
    {
      boot_log
	("Cannot open homebuy.dat. Aborting loadup of home auction data.");
      return;
    }
  ...
  for (home = first_homebuy; home; home = nexthome)
    {
      ...
      if (--home->endtime > 0
	  && ((home->bid + (home->incpercent * (home->bid / 100))) > 0))
	{
	  save_homebuy ();
	  continue;
	}
	  ...
	  && (bidder = load_player (home->bidder)) != NULL)
	{
	  if (bidder->gold < home->bid)
	    {
	      if ((bidder->gold -=
		   (home->bid / 100 * PENALTY_PERCENTAGE)) < 0)
		bidder->gold = 0;
	      home->endtime = 144;
	      STRFREE (home->bidder);
	      ...
	      if (bidder->desc)
		send_to_char
		  ("You did not have enough money for the residence you bid on.\n\r"
      ...
      if (home->seller[0] == '\0'
	  || (seller = load_player (home->seller)) == NULL)
	{
	  sprintf (buf,
		   "homebuy_update: Seller of residence with vnum %d could not be found.",
		   home->vnum);
	  bug (buf, 0);
	  ...
	  if (bidder->desc)
	    send_to_char
	      ("There was an error while looking up the seller for the residence you had\n\r"
      ...
      if (!bidder)
	{
	  ...
	  if (seller->desc)
	    send_to_char
	      ("There was no bid on your residence.  It has been removed from auction\n\r"
	...
	if (!remove_house (seller))
	  {
	    bug
	      ("homebuy_update: residence could not be removed from seller after sale.",
	       0);
	    sprintf (buf, "homebuy_update: Vnum: %d  Seller: %s   Bidder: %s",
		     home->vnum, seller->name, bidder->name);
	    bug (buf, 0);
	    logoff (bidder);
	    logoff (seller);
	    continue;
      ...
      if (!set_house (bidder, home->vnum, home->apartment))
	{
      ...
      sprintf (buf, "Homebuy_update: Home successfully sold B:%s S:%s V:%d",
	       bidder->name, seller->name, home->vnum);
      log_string (buf);

      if (!give_key (bidder, home->vnum, bidder->name, home->apartment))
	{
	  sprintf (buf,
		   "Homebuy_update: Key for residence with vnum %d could not be given.",
		   home->vnum);
	  bug (buf, 0);
	}
      ...
      if (bidder->desc)
	send_to_char
	  ("You have successfully received your new residence.\n\r", bidder);
      else
...
load_player (char *name)
{
  ...
  if (d->character->pcdata->prev_site)
    d->character->pcdata->recent_site =
      STRALLOC (d->character->pcdata->prev_site);
  d->character = NULL;
