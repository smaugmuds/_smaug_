...
*/
void
fread_liquid (LIQUID_DATA * liq, FILE * fp)
{
  ...
  for (;;)
	...
	case 'M':
	  MOD ("Mod", liq->mod[COND_DRUNK], liq->mod[COND_FULL],
	       liq->mod[COND_THIRST], liq->mod[COND_BLOODTHIRST],
	       fread_number (fp));
	  break;
...
*/
void
fread_mixture (LIQUID_MIXTURE_LIST * mix, FILE * fp)
{
  char buf[MAX_STRING_LENGTH];
  char *word;
  bool fMatch;
...
*/
void
makeliquid (CHAR_DATA * ch, char *argument)
{
  LIQUID_DATA *liq = NULL;
...
*/
void
makemixture (CHAR_DATA * ch, char *name)
{
  LIQUID_MIXTURE_LIST *mix = NULL;
  ...
  if (!found)
    {
      send_to_char
	("A liquid needs to be created before you can define mixtures to make it.\n\r",
	 ch);
      return;
    }
...
*/
void
do_setmixture (CHAR_DATA * ch, char *argument)
{
  LIQUID_MIXTURE_LIST *mix = NULL;
...
*/
void
do_setliquid (CHAR_DATA * ch, char *argument)
{
  LIQUID_DATA *liq = NULL;
  LIQUID_DATA *tliq = NULL;
  char arg[MAX_INPUT_LENGTH];
...
*/
void
do_liquids (CHAR_DATA * ch, char *argument)
{
  LIQUID_DATA *liq;
  LIQUID_MIXTURE_LIST *mix = NULL;
  bool found = FALSE;
  ...
  if (argument[0] == '\0')
    {
      ...
      pager_printf_color (ch,
			  "&G[&gVnum&G]&g--&G[&gType&G]&g--&G[&gThr&G/&gFul&G/&gDru&G/&gBlo&G]&g------&G[&gName&G]&g-------------------&G[&gShortdesc&G]&g\n\r");
      ...
      for (liq = first_liquid; liq; liq = liq->next)
	{
	  if (liq->name)
	    {
	      found = TRUE;
	      pager_printf_color (ch,
				  "&g%-8d %-7d %-3d %-3d %-3d %-3d %-24s %-10s\n\r",
				  liq->vnum, liq->type, liq->mod[COND_THIRST],
				  liq->mod[COND_FULL], liq->mod[COND_DRUNK],
				  liq->mod[3], liq->name, liq->shortdesc);
	    }
	}
      ...
      if (!found)
	pager_printf_color (ch,
			    "&RThere are currently no liquids loaded.&g\n\r");
      else
	{
	  pager_printf_color (ch,
			      "&GType 'liquids <name/vnum>' to gain more information.\n\r");
	  pager_printf_color (ch,
			      "&GType 'liquids mixtures' to list the mixture table.\n\r");
	}
  ...
  if (!str_cmp (argument, "mixtures") || !str_cmp (argument, "mixes")
      || !str_cmp (argument, "mixturetable"))
    {
      pager_printf_color (ch, "&GMixture Table\n\r");
      pager_printf_color (ch,
			  "&G[&gWith&G]&g and &G[&gWith&G]&g---&G[&gInto&G]&g---&G[&gName&G]&g\n\r");
      ...
      for (mix = first_mixture; mix; mix = mix->next)
	{
      ...
      if (!found)
	pager_printf_color (ch,
			    "&RThere are currently no mixtures loaded.\n\r");
      else
	{
	  pager_printf_color (ch,
			      "&GType 'liquids <name/vnum>' to gain more information.\n\r");
	  pager_printf_color (ch,
			      "&GType 'liquids' to list the liquid table.\n\r");
	}
      return;
    }
  ...
  if ((liq = check_liquid (argument)) == NULL)
    {
      send_to_char
	("That is not a liquid name. Type 'liquids' to get a list.\n\r", ch);
      return;
    }
  ...
  pager_printf_color (ch, "&GLiquid Information\n\r"
		      ...
		      "&GLiquid Color:&g\t  %s\n\r"
		      "&GLiquid Type:&g\t  %s\n\r", liq->name, liq->shortdesc,
		      liq->color, liquid_types[liq->type]);
  ...
  pager_printf_color (ch, "&GLiquid Vnum:&g\t  %d\n\r"
		      ...
		      "&G\tBlood:&g\t  %d\n\r"
		      "&G\tFull:&g\t	%d\n\r", liq->vnum,
		      liq->mod[COND_THIRST], liq->mod[COND_DRUNK],
		      liq->mod[3], liq->mod[COND_FULL]);
  /* mixtures  - 8/21/01 */
...
*/
LIQUID_DATA *
liq_can_mix (int vnum, LIQUID_DATA * liq2, bool is_liquid)
{
  LIQUID_MIXTURE_LIST *mix = NULL;
  OBJ_DATA *obj = NULL;
  LIQUID_DATA *liq = NULL;
...
*/
void
do_mix (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *src = NULL;
  OBJ_DATA *obj = NULL;
  LIQUID_DATA *liq = NULL;
  ...
  if ((src = get_obj_carry (ch, arg)) == NULL
      || (obj = get_obj_carry (ch, argument)) == NULL)
    {
  ...
  /* C-O-M-P-R-E-S-S ;) */
  if ((src->item_type != ITEM_DRINK_CON && src->item_type != ITEM_DRINK_MIX)
  ...
  if ((src->value[1] == 0 && src->item_type != ITEM_DRINK_MIX)
      || obj->value[1] == 0)
    {
...
do_drink (CHAR_DATA * ch, char *argument)
{
  ...
  switch (obj->item_type)
      ...
      if (obj->value[1] <= 0)
	{
      ...
      if (ch->pcdata->condition[COND_THIRST] == MAX_COND_VAL
      ...
      if (liq->type == LIQ_POISON)
	{
      ...
      if (!IS_NPC (ch))
	{
	  if (ch->pcdata->condition[COND_DRUNK] > (MAX_COND_VAL / 2)
	      && ch->pcdata->condition[COND_DRUNK] < (MAX_COND_VAL * .4))
	    send_to_char ("You feel quite sloshed.\n\r", ch);
	  else if (ch->pcdata->condition[COND_DRUNK] >= (MAX_COND_VAL * .4)
		   && ch->pcdata->condition[COND_DRUNK] < (MAX_COND_VAL * .6))
	    send_to_char ("You start to feel a little drunk.\n\r", ch);
	  else if (ch->pcdata->condition[COND_DRUNK] >= (MAX_COND_VAL * .6)
		   && ch->pcdata->condition[COND_DRUNK] < (MAX_COND_VAL * .9))
	    send_to_char ("Your vision starts to get blurry.\n\r", ch);
	  else if (ch->pcdata->condition[COND_DRUNK] >= (MAX_COND_VAL * .9)
		   && ch->pcdata->condition[COND_DRUNK] < MAX_COND_VAL)
	  ...
	  if (ch->pcdata->condition[COND_THIRST] > (MAX_COND_VAL / 2)
	      && ch->pcdata->condition[COND_THIRST] < (MAX_COND_VAL * .4))
	    send_to_char ("Your stomach begins to slosh around.\n\r", ch);
	  else if (ch->pcdata->condition[COND_THIRST] >= (MAX_COND_VAL * .4)
		   && ch->pcdata->condition[COND_THIRST] <
		   (MAX_COND_VAL * .6))
	    send_to_char ("You start to feel bloated.\n\r", ch);
	  else if (ch->pcdata->condition[COND_THIRST] >= (MAX_COND_VAL * .6)
		   && ch->pcdata->condition[COND_THIRST] <
		   (MAX_COND_VAL * .9))
	    send_to_char ("You feel bloated.\n\r", ch);
	  else if (ch->pcdata->condition[COND_THIRST] >= (MAX_COND_VAL * .9)
		   && ch->pcdata->condition[COND_THIRST] < MAX_COND_VAL)
	  ...
	  if (IS_VAMPIRE (ch))
	    {
	      if (ch->pcdata->condition[COND_BLOODTHIRST] > (MAX_COND_VAL / 2)
		  && ch->pcdata->condition[COND_BLOODTHIRST] <
		  (MAX_COND_VAL * .4))
		send_to_char_color
		...
		send_to_char
		  ("&rYou drink the last drop of the fluid, the thirst for more leaves your body.\n\r",
		   ...
		   && ch->pcdata->condition[COND_BLOODTHIRST] >= MAX_COND_VAL)
	    {
