  ...
  if ((pexit = find_door (ch, arg1, TRUE)) != NULL)
      ...
      if (IS_SET (pexit->exit_info, EX_BASHED))
	{
	  if (pexit->keyword[strlen (pexit->keyword) - 1] == 's'
	      || (pexit->keyword[strlen (pexit->keyword) - 1] == '\''
		  && pexit->keyword[strlen (pexit->keyword) - 2] == 's'))
	    act (AT_RED, "The $d have been bashed from their hinges.", ch,
	...
	case ITEM_JOURNAL:
	  {
	    ...
	    ch_printf (ch,
		       "%s has %d %s written in out of a possible %d.\n\r",
		       obj->short_descr, count, count == 1 ? "page" : "pages",
		       obj->value[0]);
...
void
do_nanny_help (CHAR_DATA * ch, char *argument)
{
  HELP_DATA *pHelp;
  char buf[MAX_STRING_LENGTH];
 ...
 */
void
do_laws (CHAR_DATA * ch, char *argument)
{
  char buf[1024];
...
//  Snippet by Senir.
sh_int
str_similarity (const char *astr, const char *bstr)
{
  sh_int matches = 0;
...
sh_int
str_prefix_level (const char *astr, const char *bstr)
{
  sh_int matches = 0;
...
void
similar_help_files (CHAR_DATA * ch, char *argument)
{
...
void
do_hset (CHAR_DATA * ch, char *argument)
{
  HELP_DATA *pHelp;
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
...
void
do_hl (CHAR_DATA * ch, char *argument)
{
...
do_who (CHAR_DATA * ch, char *argument)
{
  ...
  if (ch && argument[0] == '\0')
    {
      send_to_char_color ("\n\r&GYou must specify at least one argument.\n\r",
			  ch);
      send_to_char_color
	("Use 'who 1' to view the entire who list.  Other arguments:\n\r",
	 ch);
      send_to_char_color
  ...
  for (;;)
      ...
      else
	{
	  ...
	  if (!str_cmp (arg, "nc") || !str_cmp (arg, "newbie")
	      || !str_cmp (arg, "advisor"))
	    {
...
/* start from last to first to get it in the proper order */
  ...
  for (; d; whogr_p = whogr, whogr = (fGroup ? whogr->next : NULL),
	...
	continue;
	  ...
	  || wch->level > iLevelUpper || (fPkill && !CAN_PKILL (wch))
/*
...
*/
	...
	strcpy (char_name, wch->name);
      ...
      sprintf (class_text, "%s%2d %s", NOT_AUTHED (wch) ? "N" : " ",
	       wch->level, class_table[wch->class]->who_name);
      class = class_text;
      switch (wch->level)
	{
	default:
...
void
do_cwho (CHAR_DATA * ch, char *argument)
{
  char buf[MAX_STRING_LENGTH];
  char clan_name[MAX_INPUT_LENGTH];
  char council_name[MAX_INPUT_LENGTH];
      ...
      for (d = first_descriptor; d; d = d->next)
	{
	  if ((d->connected == CON_PLAYING || d->connected == CON_EDITING)
		       ...
		       && victim->pcdata->clan->clan_type != CLAN_GUILD)
		{
		  ...
		  pager_printf_color (ch, "&G%-*.*s\t", len, len,
				      victim->pcdata->clan->badge);
		}
	      else if (CAN_PKILL (victim))
  ...
  if (arg[0] == '\0')
      ...
      if (!IS_NPC (ch) && ch->pcdata->clan)
	{
      ...
      if (IS_IMMORTAL (ch) || (ch->pcdata->council
			       && !str_cmp (ch->pcdata->council->name,
					    "Newbie Council")))
	ch_printf_color (ch, "%s",
			 !xIS_SET (ch->deaf,
 ...
 */
void
do_retiredlist (CHAR_DATA * ch, char *argument)
{
...
do_areas (CHAR_DATA * ch, char *argument)
{
  ...
  if (arg[0] != '\0')
      ...
      if (!is_number (arg))
	{
	  ...
	  send_to_char
	    ("Area may only be followed by numbers, 'old', or an area name.\n\r",
	     ch);
	  return;
	}
	  ...
	  if (i == skill_table[sn]->skill_level[ch->class])
		...
		case POS_SITTING:
	      ...
	      pager_printf (ch,
			    ...
			    skill_table[sn]->skill_adept[ch->class], buf);
	    }
	}
    }
  return;
...
void
do_pager (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
do_ignore (CHAR_DATA * ch, char *argument)
{
  ...
  /* Prevent someone from ignoring themself... */
	   ...
	   temp = temp->next, i++)
	{
      ...
      if (stat (fname, &fst) == -1 &&
	  (!(victim = get_char_world (ch, arg)) ||
	   IS_NPC (victim) || strcmp (capitalize (arg), victim->name) != 0))
	{
	  set_char_color (AT_IGNORE, ch);
