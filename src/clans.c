...
bool
load_council_file (char *councilfile)
{
  char filename[256];
  COUNCIL_DATA *council;
  FILE *fp;
...
do_make (CHAR_DATA * ch, char *argument)
{
  ...
  if (str_cmp (ch->name, clan->leader)
      ...
      && (clan->clan_type != CLAN_GUILD || str_cmp (ch->name, clan->number1)))
    {
      send_to_char ("Huh?\n\r", ch);
      return;
    }
...
void
do_council_induct (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  COUNCIL_DATA *council;
...
do_outcast (CHAR_DATA * ch, char *argument)
{
  ...
  if (x <= y && (get_trust (ch) <= get_trust (victim)))
    {
      send_to_char
	("You are not powerful enough to outcast this character.\n\r", ch);
      return;
    }
...
void
do_council_outcast (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  COUNCIL_DATA *council;
...
void
do_setclan (CHAR_DATA * ch, char *argument)
{
...
void
do_setcouncil (CHAR_DATA * ch, char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  COUNCIL_DATA *council;
  ...
  if (arg1[0] == '\0')
    {
      send_to_char
	("Usage: setcouncil <council> <field> <deity|leader|number1|number2> <player>\n\r",
	 ch);
      send_to_char ("\n\rField being one of:\n\r", ch);
...
void
do_showclan (CHAR_DATA * ch, char *argument)
{
  CLAN_DATA *clan;
...
void
do_showcouncil (CHAR_DATA * ch, char *argument)
{
  COUNCIL_DATA *council;
...
void
do_makeclan (CHAR_DATA * ch, char *argument)
{
  char filename[256];
  CLAN_DATA *clan;
  bool found;
...
void
do_makecouncil (CHAR_DATA * ch, char *argument)
{
  char filename[256];
  COUNCIL_DATA *council;
  bool found;
...
void
do_orders (CHAR_DATA * ch, char *argument)
{
  CLAN_DATA *order;
  int count = 0;
...
void
do_councils (CHAR_DATA * ch, char *argument)
{
  COUNCIL_DATA *council;
  ...
  if (argument[0] == '\0')
      ...
      for (council = first_council; council; council = council->next)
	{
      ...
      send_to_char_color
	("&GUse 'councils <name of council>' for more detailed information.\n\r",
	 ch);
      return;
    }
  council = get_council (argument);
...
void
do_guilds (CHAR_DATA * ch, char *argument)
{
  char buf[MAX_STRING_LENGTH];
  CLAN_DATA *guild;
  int count = 0;
...
void
do_defeats (CHAR_DATA * ch, char *argument)
{
  char filename[256];
...
void
do_victories (CHAR_DATA * ch, char *argument)
{
  char filename[256];
...
void
show_members (CHAR_DATA * ch, char *argument)
{
  ...
  pager_printf_color (ch,
		      "&G::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::\n\r");
  pager_printf_color (ch,
		      "&GLvl  Name          Class       Kills   Deaths        Entered\n\r\n\r");
  for (member = members_list->first_member; member; member = member->next)
    {
      if (str_cmp (member->name, clan->leader)
	  && str_cmp (member->name, clan->number1)
	  && str_cmp (member->name, clan->number2))
	pager_printf_color (ch,
			    "&G%3d  &w%-12s  &G%-10s %6d  %7d     &g%10s\n\r",
			    member->level, capitalize (member->name),
...
void
remove_member (char *clanname, char *membername)
{
  MEMBER_LIST *members_list;
  MEMBER_DATA *member;
...
void
add_member (CHAR_DATA * ch, char *clanname)
{
  MEMBER_LIST *members_list;
  ...
  LINK (nmember, members_list->first_member, members_list->last_member, next,
	prev);
...
void
do_memberlist (CHAR_DATA * ch, char *argument)
{
  ...
  if (!IS_IMMORTAL (ch))
      ...
      if (!str_cmp (arg1, "create"))
	{
	  ...
	  LINK (members_list, first_member_list, last_member_list, next,
		prev);
	  save_member_lists ();
      ...
      if (!str_cmp (arg1, "delete"))
	{
	  ...
	  for (members_list = first_member_list; members_list;
		...
		UNLINK (members_list, first_member_list, last_member_list,
			next, prev);
...
save_member_lists ()
{
  ...
  for (members_list = first_member_list; members_list;
	...
	fprintf (fp, "Member        %s %s %d %d %d %d\n", member->name,
		 member->since, member->kills, member->deaths, member->level,
		 member->class);
      fprintf (fp, "End\n\n");
    }
...
#define PURGEROSTER
      if (!str_cmp (word, "Member"))
	{
	  ...
	  LINK (member, members_list->first_member, members_list->last_member,
		next, prev);
	  continue;
	}
...
update_member (CHAR_DATA * ch)
{
  ...
  for (members_list = first_member_list; members_list;
	...
	{
	  for (member = members_list->first_member; member;
	       member = member->next)
	    {
	      if (!str_cmp (member->name, ch->name))
		{
