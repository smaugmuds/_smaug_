/*
 ...
 [|             /  *********   *        *  *        *   ********    *******   |]
  \\____________\____________________________________________________________//
     ...
     |             Last Modified:  Feb. 24, 1999  (Mystaric)               |
     |_____________________________________________________________________|
    //                                                                     \\
...
void
toadminfile (const char *line, FILE * wfp)
{
  int filler, xx;
...
void
add_to_adminlist (char *name, int level)
{
  ADMINENT *admin;
...
make_adminlist ()
{
  ...
  for (admin = first_admin; admin; admin = admin->next)
    {
      sprintf (buf, "%s%c/%s", PLAYER_DIR, tolower (admin->name[0]),
	       admin->name);
      gfp = fopen (buf, "r");
      if (gfp)
	{
...
void
do_makeadminlist (CHAR_DATA * ch, char *argument)
{
  make_adminlist ();
}
	  ...
	  if (tmp)
	    {
	      for (council = first_council; council; council = council->next)
		{
		  if (!str_cmp (council->head, admin->name)
		      || (council->head2
			  && !str_cmp (council->head2, admin->name)))
		    head = TRUE;
		  if (!str_cmp (council->head, tmp->name)
		      || (council->head2
...
void
do_adminlist (CHAR_DATA * ch, char *argument)
{
