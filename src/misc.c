/*
 ...
 [|             /  *********   *        *  *        *   ********    *******   |]
  \\____________\____________________________________________________________//
     ...
     |_____________________________________________________________________|
     |                                                                     |
     | 					       General Commands: not skills or spells              |
     |_____________________________________________________________________|
     |                                                                     |
     |            Most of the stuff in here would go in act_obj.c,         |
     |                    but but act_obj was getting big.                 |
     |_____________________________________________________________________|
    //                                                                     \\
...
void
do_rap (CHAR_DATA * ch, char *argument)
{
  EXIT_DATA *pexit;
  char arg[MAX_INPUT_LENGTH];
...
void
do_tamp (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *pipe;
  char arg[MAX_INPUT_LENGTH];
...
void
do_smoke (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *pipe;
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!IS_SET (pipe->value[3], PIPE_LIT))
    {
      act (AT_ACTION, "You try to smoke $p, but it's not lit.", ch, pipe,
	   NULL, TO_CHAR);
      act (AT_ACTION, "$n tries to smoke $p, but it's not lit.", ch, pipe,
	   NULL, TO_ROOM);
      return;
    }
  if (pipe->value[1] > 0)
	...
	{
	  act (AT_ACTION, "You draw thoughtfully from $p.", ch, pipe, NULL,
	       TO_CHAR);
	  act (AT_ACTION, "$n draws thoughtfully from $p.", ch, pipe, NULL,
	       TO_ROOM);
	}
...
void
do_extinguish (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *obj;
  char arg[MAX_INPUT_LENGTH];
  ...
  if ((obj = get_obj_wear (ch, arg)) == NULL)
    if ((obj = get_obj_carry (ch, arg)) == NULL)
      if ((obj =
	   get_obj_list_rev (ch, arg, ch->in_room->last_content)) == NULL)
	{
	  send_to_char ("You aren't carrying that.\n\r", ch);
	  return;
	}
...
void
do_light (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *obj;
  char arg[MAX_INPUT_LENGTH];
  ...
  switch (obj->item_type)
      ...
      if (!IS_SET (obj->value[3], PIPE_LIT))
	{
	  if (obj->value[1] < 1)
	    {
	      act (AT_ACTION, "You try to light $p, but it's empty.", ch, obj,
		   NULL, TO_CHAR);
	      act (AT_ACTION, "$n tries to light $p, but it's empty.", ch,
		   obj, NULL, TO_ROOM);
	      return;
	    }
      ...
      if (obj->value[1] > 0)
	{
	  if (!IS_SET (obj->value[3], PIPE_LIT))
	    {
	      act (AT_ACTION, "You carefully light $p.", ch, obj, NULL,
		   TO_CHAR);
	      act (AT_ACTION, "$n carefully lights $p.", ch, obj, NULL,
		   TO_ROOM);
	      SET_BIT (obj->value[3], PIPE_LIT);
	    }
	  else
 ...
 */
void
do_apply (CHAR_DATA * ch, char *argument)
{
...
void
actiondesc (CHAR_DATA * ch, OBJ_DATA * obj, void *vo)
{
