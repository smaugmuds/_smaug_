...
do_morphset (CHAR_DATA * ch, char *argument)
{
  ...
  if (arg1[0] == '\0' || (arg2[0] == '\0' && ch->substate != SUB_REPEATCMD)
	...
	("&c-------------------------------------------------\n\r", ch);
      send_to_char
	("  ac, affected, blood, bloodused, cha, class, con, damroll, dayto,\n\r",
	 ch);
      send_to_char ("  dayfrom, deity, description, defpos, dex, dodge,\n\r",
		    ch);
      send_to_char
	("  favourused, gloryused, help, hitroll, hp, hpused, immune,\n\r",
	 ch);
      send_to_char
	...
	("  morphother, morphself, move, moveused, name, noaffected,\n\r",
	 ch);
      send_to_char
	("  nocast, noimmune, noresistant, noskill, nosusceptible,\n\r", ch);
      send_to_char
  ...
  if (argument[0] == '\0')
	...
	("&B[----------------------------------------------------------------------------]\n\r",
	 ch);
      ...
      pager_printf_color (ch,
			  "  &cLevel:  &C%d       &cCasting Allowed   : &C%s\n\r",
			  morph->level, (morph->no_cast) ? "NO" : "yes");
      pager_printf_color (ch,
			  "  &cUSAGES:  Mana:  &C%d  &cMove:  &C%d  &cHp:  &C%d  &cFavour:  &C%d  &cGlory:  &C%d\n\r",
	...
	("&B[----------------------------------------------------------------------------]\n\r",
	 ch);
      ...
      pager_printf_color (ch,
			  ...
			  morph->saving_spell_staff, morph->dodge,
			  morph->parry, morph->tumble);
      pager_printf_color (ch,
 ...
 */
MORPH_DATA *
find_morph (CHAR_DATA * ch, char *target, bool is_cast)
{
  MORPH_DATA *morph;
  if (target[0] == '\0')
    return NULL;
...
fread_morph (FILE * fp)
{
  ...
  for (;;)
	...
	case 'T':
	  ...
	  break;
	case 'U':
	  KEY ("UnmorphOther", morph->unmorph_other,
