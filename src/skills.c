...
void
do_skin (CHAR_DATA * ch, char *argument)
{
  ...
  if ((obj->value[3] != 1
       && obj->value[3] != 2
       && obj->value[3] != 3
       && obj->value[3] != 11) || IS_OBJ_STAT (corpse, ITEM_SKINNED))
    {
  ...
  if (!IS_IMMORTAL (ch))
      ...
      if (IS_OBJ_STAT (corpse, ITEM_CLANCORPSE))
	{
	  if (corpse->action_desc && is_name (ch->name, corpse->action_desc));
	    ...
	    {
	      send_to_char
		("There is nothing you can do with this corpse.\n\r", ch);
	      return;
	    }
	}
      else
...
void
do_grapple (CHAR_DATA * ch, char *argument)
{
  ...
  if ((victim = who_fighting (ch)) == NULL)
	...
	{
	  act (AT_ACTION,
	       "You move in a circle looking for someone to grapple.", ch,
	       NULL, NULL, TO_CHAR);
	  act (AT_ACTION,
	       "$n moves in a circle looking for someone to grapple.", ch,
	       NULL, NULL, TO_ROOM);
	  return;
	}
  ...
  if (!chance (ch, percent))
    {
      ...
      act (AT_ACTION, "$n tries to grapple you but can't get close enough.",
...
*/
  ...
  act (AT_ACTION, "$n begins grappling with $N!", ch, NULL, victim,
       TO_NOTVICT);
...
void
do_detrap (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  OBJ_DATA *trap;
...
void
do_dig (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  OBJ_DATA *startobj;
...
*/
	  if (can_use_skill (ch, (number_percent () * (shovel ? 1 : 4)),
	      ...
	      else
		{
		  ...
		  act (AT_PLAIN, "$n uncovers a doorway!", ch, NULL, NULL,
		       TO_ROOM);
		}
...
void
do_pounce (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  OBJ_DATA *obj;
  ...
  if ((obj = get_eq_char (ch, WEAR_WIELD)) == NULL
	  ...
	  && obj->value[3] != 3
	  && obj->value[3] != 5
	  && obj->value[3] != 10 && obj->value[3] != 11))
    {
      send_to_char
	("You are not wielding an appropriate weapon type to effectively pounce.\n\r",
	 ch);
      return;
    }
...
void
do_meditate (CHAR_DATA * ch, char *argument)
{
  char *arg;
  int percent;
  ...
  switch (ch->substate)
      ...
      if (IS_NPC (ch) || IS_AFFECTED (ch, AFF_CHARM))
	{
      ...
      send_to_char_color
	("&BYou enter into a meditative state, hoping to collect mana from the cosmos.\n\r",
	 ch);
      add_timer (ch, TIMER_DO_FUN,
		 UMAX (2, (skill_table[gsn_meditate]->beats / 8)),
		 do_meditate, 1);
  ...
  if (can_use_skill (ch, percent, gsn_meditate))
    {
      ...
      send_to_char_color
	("&BYou meditate peacefully, collecting mana from the cosmos.\n\r",
	 ch);
    ...
    {
      send_to_char_color
	("&BYou spend several minutes in deep concentration, but fail to collect any mana.\n\r",
	 ch);
...
do_trance (CHAR_DATA * ch, char *argument)
{
  ...
  switch (ch->substate)
      ...
      if (IS_NPC (ch) || IS_AFFECTED (ch, AFF_CHARM))
	{
      ...
      send_to_char_color
	("&BYou enter a peaceful trance, collecting mana from the cosmos.\n\r",
	 ch);
      add_timer (ch, TIMER_DO_FUN,
		 UMAX (2, (skill_table[gsn_trance]->beats / 8)), do_trance,
		 1);
      ch->alloc_ptr = str_dup (argument);
      return;
  ...
  if (can_use_skill (ch, percent, gsn_trance))
    ...
    {
      send_to_char_color
	("&BYou spend several minutes in a deep trance, but fail to collect any mana.\n\r",
	 ch);
...
void
do_claw (CHAR_DATA * ch, char *argument)
{
  CHAR_DATA *victim;
...
void
do_sting (CHAR_DATA * ch, char *argument)
{
  CHAR_DATA *victim;
...
void
do_tail (CHAR_DATA * ch, char *argument)
{
  CHAR_DATA *victim;
...
void
do_bloodlet (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *obj;
...
/* Shargate, May 2002 */
void
do_cleave (CHAR_DATA * ch, char *argument)
{
  CHAR_DATA *victim;
  OBJ_DATA *obj;
  int dam = 0;
...
void
do_mistwalk (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  bool allowday;
...
void
do_broach (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  EXIT_DATA *pexit;
...
do_recall (CHAR_DATA * ch, char *argument)
{
  ...
  if ((opponent = who_fighting (ch)) != NULL)
      ...
      if (number_bits (1) == 0 || (!IS_NPC (opponent) && number_bits (3) > 1))
	{
	  WAIT_STATE (ch, 4);
	  lose =
	    (exp_level (ch, ch->level + 1) - exp_level (ch, ch->level)) * 0.1;
	  if (ch->desc)
	    lose /= 2;
	  gain_exp (ch, 0 - lose);
      ...
      lose =
	(exp_level (ch, ch->level + 1) - exp_level (ch, ch->level)) * 0.2;
      if (ch->desc)
	lose /= 2;
      gain_exp (ch, 0 - lose);
...
void
do_aid (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  int percent;
  ...
  if (IS_NPC (victim))
    {
      ...
      if (victim->pIndexData->stances[victim->stance] >
	  (STANCE_GRAND_MASTER * .75))
	...
	return FALSE;
      ...
      if (victim->pcdata->stances[victim->stance] >
	  (STANCE_GRAND_MASTER * .75))
	chances += stance_index[victim->stance].parry;
      else
	chances += (stance_index[victim->stance].parry / 2);
...
void
do_poison_weapon (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *obj;
  OBJ_DATA *pobj;
  OBJ_DATA *wobj;
...
void
do_hitall (CHAR_DATA * ch, char *argument)
{
  CHAR_DATA *vch;
  CHAR_DATA *vch_next;
  sh_int nvict = 0;
 ...
 */
CHAR_DATA *
scan_for_victim (CHAR_DATA * ch, EXIT_DATA * pexit, char *name)
{
  CHAR_DATA *victim;
  ROOM_INDEX_DATA *was_in_room;
  sh_int dist, dir;
 ...
 */
ch_ret
ranged_attack (CHAR_DATA * ch, char *argument, OBJ_DATA * weapon,
  ...
  while (dist <= range)
	  ...
	  if (projectile)
	    {
	      ...
	      act (color,
		   "$p flies in from $T and falls harmlessly to the ground.",
      ...
      if ((pexit = get_exit (ch->in_room, dir)) == NULL)
	{
	  if (projectile)
	    {
	      act (color,
		   "Your $t hits a wall and bounces harmlessly to the $T.",
		   ch, myobj (projectile), dir_name[dir], TO_CHAR);
	      act (color, "$p falls harmlessly to the ground.", ch,
		   projectile, dir_name[dir], TO_ROOM);
	      if (projectile->in_obj)
		obj_from_obj (projectile);
	      if (projectile->carried_by)
		obj_from_char (projectile);
	  ...
	  else
	    {
	      ...
	      act (color,
		   "$t strikes the $Tern wall and falls harmlessly to the ground.",
 ...
 */
void
do_fire (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *arrow;
  OBJ_DATA *bow;
  sh_int max_dist;
 ...
 */
bool
mob_fire (CHAR_DATA * ch, char *name)
{
  OBJ_DATA *arrow;
  OBJ_DATA *bow;
  sh_int max_dist;
...
void
do_slice (CHAR_DATA * ch, char *argument)
{
  OBJ_DATA *corpse;
  OBJ_DATA *obj;
  OBJ_DATA *slice;
 ...
 */ void
do_style (CHAR_DATA * ch, char *argument)
{
  char arg[MAX_INPUT_LENGTH];
