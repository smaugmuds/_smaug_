 ...
 */
int
str_count (char *psource, char *ptarget)
{
  char *ptemp = psource;
  int count = 0;
 ...
 */
void
do_opfind (CHAR_DATA * ch, char *argument)	/* Gorog */
{
  OBJ_INDEX_DATA *pObj;
  MPROG_DATA *pProg;
...
void
do_mpfind (CHAR_DATA * ch, char *argument)	/* Gorog */
{
  MOB_INDEX_DATA *pMob;
  MPROG_DATA *pProg;
  char arg1[MAX_INPUT_LENGTH];
 ...
 */
void
do_rpfind (CHAR_DATA * ch, char *argument)	/* Gorog */
{
...
int
rgrub_int_comp (const void *i, const void *j)
{
  return *(int *) i - *(int *) j;
}

void
rgrub_help (CHAR_DATA * ch)
{
  send_to_char ("Syntax:\n\r", ch);
  ...
  send_to_char
    ("   e.g. rgrub f nomob 901 969 - list all rooms in Olympus\n\r"
...
void
do_rgrub (CHAR_DATA * ch, char *argument)
{
      ...
      for (cou = 0; cou < MAX_KEY_HASH; cou++)
	{
	  if (room_index_hash[cou])
		...
		if (pRoom->vnum >= lo && pRoom->vnum <= hi)
		  {
		    if (match == pRoom->sector_type
			&& hit_cou < RGRUB_MAX_SIZE)
...
short
go_wear_ext (long arg)		/* extract bit set in arg ignoring pos 1 */
{
  short cou;
  if (arg <= 1)
    return arg;
...
int
go_strcmp (const char *astr, const char *bstr)
{
  int i;
 ...
 */
void
go_sort (CHAR_DATA * ch, GO_STRUCT ** p,
...
void
go_accum_aff (GO_STRUCT * r, int loc, int mod)
{
  enum
...
void
display_operand_table (CHAR_DATA * ch, int op_num)
{
  int cou;
  ...
  for (cou = 0; cou < op_num; cou++)
    if (go_op[cou].num)
      pager_printf (ch,
		    "%2d %-7s %2s %10ld\n\r", cou + 1,
		    go_fd[go_op[cou].field].nam, opn[go_op[cou].op],
		    go_op[cou].nval);
    else
 ...
 */
bool
go_parse_operator (CHAR_DATA * ch, char *pch, int *op_num)
{
  enum op_type
 ...
 */
bool
go_parse_operand (CHAR_DATA * ch, char *arg, int *op_num, int *sor_ind,
		  bool * sor_dir, bool * or_sw, bool * np_sw, bool * nm_sw,
		  bool * ng_sw, bool * do_sw, bool * d2_sw)
{
  int cou;
  char *pch;
 ...
 */
bool
go_eval_str (char *lval, int op, char *rval)
{
  enum op_type
  { EQ, NE, SU, GE, GT, LE, LT };
  switch (op)
 ...
 */
bool
go_eval_num (long lval, int op, long rval)
{
  enum op_type
  { EQ, NE, SU, GE, GT, LE, LT };
  switch (op)
 ...
 */
bool
go_eval_and (CHAR_DATA * ch, GO_STRUCT * r, int op_num)
{
  enum
  { OCOUNT, OVNUM, OTYPE, OLEVEL, OWEAR, OAVG, OHR, ODR, OHP, OMP, OAC,
    OSTR, ODEX, OCON, OWIS, OINT, OLUCK,
 ...
 */
bool
go_eval_or (CHAR_DATA * ch, GO_STRUCT * r, int op_num)
{
  enum
  { OCOUNT, OVNUM, OTYPE, OLEVEL, OWEAR, OAVG, OHR, ODR, OHP, OMP, OAC,
    OSTR, ODEX, OCON, OWIS, OINT, OLUCK,
...
void
go_display (CHAR_DATA * ch, int dis_num, int tot_match, bool d2_sw,
	    GO_STRUCT ** p)
{
  enum
  { OCOUNT, OVNUM, OTYPE, OLEVEL, OWEAR, OAVG, OHR, ODR, OHP, OMP, OAC,
 ...
 */
bool
go_read_names (CHAR_DATA * ch, OBJ_DATA * po, GO_STRUCT * r, bool np_sw,
...
bool
go_read (CHAR_DATA * ch, int dis_num, int op_num, int sor_ind,
	 ...
	 bool d2_sw)
{
  ...
  for (po = first_object; po; po = po->next)	/* Loop through all objects   */
	...
	go_accum_aff (&r, pa->location, pa->modifier);
      res =
	or_sw ? go_eval_or (ch, &r, op_num) : go_eval_and (ch, &r, op_num);
...
void
do_ogrub (CHAR_DATA * ch, char *argument)
{
  enum
  { OCOUNT, OVNUM, OTYPE, OLEVEL, OWEAR, OAVG, OHR, ODR, OHP, OMP, OAC,
    OSTR, ODEX, OCON, OWIS, OINT, OLUCK,
  ...
  if (isdigit (*arg1))		/* first arg is number of display lines   */
    ...
    {
      pager_printf (ch,
		    "You did not specify the number of display lines.\n\r");
      return;
    }
  ...
  while (*arg1)			/* build the operand table        */
	...
	{
	  pager_printf (ch,
			"Sorry. You have entered more than %d operands.\n\r",
...
char *
gr_strc (char c)		/* convert a char to a str */
{
  static char s[2] = "s";
  s[0] = c;
  return s;
 ...
 */
bool
gr_eval_and (GR_STRUCT r, int op_num)
{
  int cou;
  for (cou = 0; cou < op_num; cou++)
    {
	...
	case pkill:
	  if (!go_eval_str
	      (gr_strc (r.pkill), gr_op[cou].op, gr_op[cou].sval))
	    return FALSE;
	  else
	    break;
	}
 ...
 */
bool
gr_eval_or (GR_STRUCT r, int op_num)
{
  int cou;
  for (cou = 0; cou < op_num; cou++)
    {
 ...
 */
bool
gr_parse_operator (CHAR_DATA * ch, char *pch, int *op_num)
{
  enum op_type
  { EQ, NE, SU, GE, GT, LE, LT };
  int cou;
 ...
 */
bool
gr_parse_operand (CHAR_DATA * ch, char *arg, bool * or_sw, int *op_num)
{
  int cou;
 ...
 */
void
gr_read (CHAR_DATA * ch, int op_num, bool or_sw, int dis_num)
{
  FILE *fp;
  bool res;			/* result of a boolean exp   */
 ...
 */
void
do_grub (CHAR_DATA * ch, char *argument)
{
  char arg1[MAX_STRING_LENGTH];
  bool or_sw = FALSE;		/* or search criteria           */
  int dis_num;			/* display lines requested      */
  ...
  if (!*arg1)
    {
      ch_printf (ch,
		 "Please type HELP GRUB for info on how to use GRUB.\n\r");
      return;
    }
...
void
do_showlayers (CHAR_DATA * ch, char *argument)
{
  extern OBJ_INDEX_DATA *obj_index_hash[MAX_KEY_HASH];
  OBJ_INDEX_DATA *pObj;
  char arg1[MAX_STRING_LENGTH];
  ...
  if (!*arg1 || (!is_number (arg1) && str_cmp (arg1, "layer")))
    {
      ...
      send_to_char ("Syntax:\n\r", ch);
      send_to_char
	("showlayers n <w> -  display maximum of n lines <with w wear location.>\n\r",
	 ch);
      send_to_char
	("showlayers layer ## - Show all of the sublayers a particular layer number uses.  \n\r",
	 ch);
      send_to_char (" Example: showlayers 30 - Show 30 layerable items.\n\r",
		    ch);
      send_to_char
	("          showlayers 30 arms  - Show 30 items layerable on arms wearloc\n\r",
	 ch);
      send_to_char
	("          showlayers layer 126  - Displays each of the sublayers to 126\n\r",
	 ch);
      return;
    }
  ...
  if (!str_cmp (arg1, "layer"))
      ...
      if (argnum < 1 || argnum > 128)
	{
      ...
      ch_printf_color (ch,
		       " &GThe layer number %d uses the following sub layers:\n\r",
		       argnum);
  ...
  send_to_pager_color
    ("   &B#  &cVnum      &WWear &YLayer   &CDescription\n\r", ch);
  for (hash = 0; hash < MAX_KEY_HASH; hash++)	/* loop thru obj_index_hash */
      ...
      if (obj_index_hash[hash])
	{
	  for (pObj = obj_index_hash[hash]; pObj; pObj = pObj->next)
		...
		{
		  if (arg2[0] == '\0'
		      || IS_SET (pObj->wear_flags, 1 << get_wflag (arg2)))
 ...
 */
void
zero_sort (int *vnums, int *count, int left, int right)
{
...
void
diag_visit_obj (CHAR_DATA * ch, OBJ_DATA * obj)
{
...
void
diag_nivek_obj (CHAR_DATA * ch, OBJ_DATA * obj)
{
  pager_printf (ch, "***obj=%s\n\r", obj->name);
  if (obj->prev_content)
...
int
diag_int_comp (const void *i, const void *j)
{
  return *(int *) i - *(int *) j;
}
...
diagnose_help (CHAR_DATA * ch)
{
  ...
  send_to_char
    ("   e.g. diagnose rf 6 901 969 - list all rooms in Olympus\n\r"
     "      that are nomob and deathtraps.\n\r", ch);
  send_to_char ("   e.g. diagnose rf 2 - list all deathtraps.\n\r", ch);
  send_to_char
    ("diagnose mrc num race class vnum1 vnum2 - mobs/race/class\n\r"
...
void
diag_ins (OBJ_INDEX_DATA * p, int siz, OBJ_INDEX_DATA ** f, CHAR_DATA * ch)
{
  ...
  if (!str_cmp (arg1, "clones"))
      ...
      for (dsrc = first_descriptor; dsrc; dsrc = dsrc_next)
	{
	  ...
	  for (ddst = first_descriptor; ddst; ddst = ddst_next)
		...
		{
		  UNLINK (ddst, first_descriptor, last_descriptor, next,
			  prev);
	...
	for (pMob = mob_index_hash[hash]; pMob; pMob = pMob->next)
	  {
	    if (pMob->count < mcount)
	      ...
	      {
		ch_printf (ch, "%5d\t\t%d\t\t%s\n\r",
			   pMob->count, pMob->vnum, pMob->player_name);
		totcount++;
	      }
	  }
