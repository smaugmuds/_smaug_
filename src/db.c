...
#ifdef OVERLANDCODE
  log_string (_("Loading Overland maps ..."));
  load_maps ();
#endif
...
*/
   ...
   */
  {
    ...
    area_update ();
#ifdef OVERLANDCODE
		log_string( _("Loading Overland resets ...") );
		load_mapresets();
#endif
  ...
  for (;;)
	...
	case 'B':
	  switch (arg2 & BIT_RESET_TYPE_MASK)
		...
		if (!pRoomIndex)
		  {
		    ...
		    if (fBootDb)
		      boot_log
			("Load_resets: %s (%d) 'B': room %d doesn't exist.",
			 tarea->filename, count, arg1);
		  }

		door =
		  (arg2 & BIT_RESET_DOOR_MASK) >> BIT_RESET_DOOR_THRESHOLD;
		...
		if (get_obj_index (arg1) == NULL && fBootDb)
		  boot_log
		    ("Load_resets: %s (%d) 'B': object %d doesn't exist.",
		     tarea->filename, count, arg1);
	      break;
		...
		if (get_mob_index (arg1) == NULL && fBootDb)
		  boot_log
		    ("Load_resets: %s (%d) 'B': mobile %d doesn't exist.",
		     tarea->filename, count, arg1);
	      break;
	    default:
	  ...
	  for (pch = first_char; pch; pch = pch->next)
	    {
	      if (!IS_NPC (pch)
		  && IS_AWAKE (pch)
		  && pch->in_room && pch->in_room->area == pArea)
		{
...
create_mobile (MOB_INDEX_DATA * pMobIndex)
{
  ...
  if (!pMobIndex->hitnodice)
    mob->max_hit =
      mob->level * 70 + number_range (mob->level * mob->level / 4,
				      mob->level * mob->level);
  else
    mob->max_hit =
 ...
 */
void
append_to_file (char *file, char *str)
{
  FILE *fp;
 ...
 */
void
show_file (CHAR_DATA * ch, char *filename)
{
  FILE *fp;
  char buf[MAX_STRING_LENGTH];
  int c;
 ...
 */
void
show_file_vnum (CHAR_DATA * ch, char *filename, int lo, int hi)
{
  FILE *fp;
  char buf[MAX_STRING_LENGTH];
  int c;
 ...
 */
void
do_dmesg (CHAR_DATA * ch, char *argument)
{
...
void
do_makeretiredlist (CHAR_DATA * ch, char *argument)
{
  make_retiredlist ();
}
  ...
  for (wiz = first_wiz; wiz; wiz = wiz->next)
      ...
      if (wiz->level < ilevel)
	{
	  ...
	  switch (ilevel)
	    {
	    case MAX_LEVEL - 0:
	      towizfile ("     &GSupreme Entity&W");
	      break;
	    case MAX_LEVEL - 1:
	      towizfile ("     &GInfinite&W");
	      break;
	    case MAX_LEVEL - 2:
	      towizfile ("     &GEternal&W");
	      break;
	    case MAX_LEVEL - 3:
	      towizfile ("     &GAncient&W");
	      break;
	    case MAX_LEVEL - 4:
	      towizfile ("     &GExalted Gods&W");
	      break;
	    case MAX_LEVEL - 5:
	      towizfile ("     &GAscendant Gods&W");
	      break;
	    case MAX_LEVEL - 6:
	      towizfile ("     &GGreater Gods&W");
	      break;
	    case MAX_LEVEL - 7:
	      towizfile ("     &GGods&W");
	      break;
	    case MAX_LEVEL - 8:
	      towizfile ("     &GLesser Gods&W");
	      break;
	    case MAX_LEVEL - 9:
	      towizfile ("     &GImmortals&W");
	      break;
	    case MAX_LEVEL - 10:
	      towizfile ("     &GDemi Gods&W");
	      break;
	    case MAX_LEVEL - 11:
	      towizfile ("     &GSaviors&W");
	      break;
	    case MAX_LEVEL - 12:
	      towizfile ("     &GCreators&W");
	      break;
	    case MAX_LEVEL - 13:
	      towizfile ("     &GAcolytes&W");
	      break;
	    case MAX_LEVEL - 14:
	      towizfile ("     &GNeophytes&W");
	      break;
	    case MAX_LEVEL - 16:
	      towizfile ("     &GGuests&W");
	      break;
	    default:
	      towizfile ("     &GServants&W");
	      break;
	    }
	}
      if (strlen (buf) + strlen (wiz->name) > 76)
...
void
do_makewizlist (CHAR_DATA * ch, char *argument)
{
  make_wizlist ();
}
...
MPROG_DATA *
mprog_file_read (char *f, MPROG_DATA * mprg, MOB_INDEX_DATA * pMobIndex)
{
 ...
 */
void
load_mudprogs (AREA_DATA * tarea, FILE * fp)
{
  MOB_INDEX_DATA *iMob;
  MPROG_DATA *original;
  MPROG_DATA *working;
...
MPROG_DATA *
oprog_file_read (char *f, MPROG_DATA * mprg, OBJ_INDEX_DATA * pObjIndex)
{
 ...
 */
void
load_objprogs (AREA_DATA * tarea, FILE * fp)
{
  OBJ_INDEX_DATA *iObj;
  MPROG_DATA *original;
  MPROG_DATA *working;
...
void
oprog_read_programs (FILE * fp, OBJ_INDEX_DATA * pObjIndex)
{
  MPROG_DATA *mprg;
  char letter;
  bool done = FALSE;
...
/* This routine reads in scripts of OBJprograms from a file */
MPROG_DATA *
rprog_file_read (char *f, MPROG_DATA * mprg, ROOM_INDEX_DATA * RoomIndex)
{
 ...
 */
void
load_roomprogs (AREA_DATA * tarea, FILE * fp)
{
...
void
rprog_read_programs (FILE * fp, ROOM_INDEX_DATA * pRoomIndex)
{
  MPROG_DATA *mprg;
  char letter;
  bool done = FALSE;
 ...
 */
OBJ_INDEX_DATA *
make_object (int vnum, int cvnum, char *name)
{
 ...
 */
MOB_INDEX_DATA *
make_mobile (int vnum, int cvnum, char *name)
{
...
void
load_area_file (AREA_DATA * tarea, char *filename)
{
...
void
sort_noauctions (NOAUCTION_DATA * pNoauc)
{
 ...
 */
void
do_vnums (CHAR_DATA * ch, char *argument)
{
 ...
 */
void
do_zones (CHAR_DATA * ch, char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  int low, high;
 ...
 */
void
do_newzones (CHAR_DATA * ch, char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  int low, high;
...
init_area_weather ()
{
  ...
  for (pArea = first_area; pArea; pArea = pArea->next)
      ...
      for (neigh = pArea->weather->first_neighbor; neigh; neigh = next_neigh)
	{
	  ...
	  /* area does not exist */
	  if (!neigh->address || str_cmp (neigh->name, neigh->address->name))
	    {
	      tneigh = neigh;
	      next_neigh = tneigh->next;
	      UNLINK (tneigh,
		      pArea->weather->first_neighbor,
		      pArea->weather->last_neighbor, next, prev);
	      STRFREE (tneigh->name);
	      DISPOSE (tneigh);
...
PROJECT_DATA *
read_project (char *filename, FILE * fp)
{
  PROJECT_DATA *project;
  NOTE_DATA *log, *tlog;
  char *word;
  ...
  for (;;)
	...
	case 'E':
	  if (!str_cmp (word, "End"))
		...
		{
		  sprintf (checkname, "%s%c/%s", PLAYER_DIR,
			   tolower (lmsg->name[0]), capitalize (lmsg->name));

		  if (access (checkname, F_OK) != 0)
		    {
		      bug
			("load_loginmsg: Login message expired - %s no longer exists",
			 lmsg->name);
		      STRFREE (lmsg->name);
		      STRFREE (lmsg->text);
		      DISPOSE (lmsg);
...
void
add_loginmsg (char *name, sh_int type, char *argument)
{
  LMSG_DATA *lmsg;
  ...
  for (lmsg = first_lmsg; lmsg; lmsg = lmsg_next)
      ...
      if (!str_cmp (lmsg->name, ch->name))
	{
	  ...
	  if (lmsg->type > MAX_MSG)
	    {
	      sprintf (buf, "Error: Unknown login msg: %d for %s.",
		       lmsg->type, ch->name);
	      bug (buf, 0);
	    }
